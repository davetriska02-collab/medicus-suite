...
const HOUR_START = 6,
  HOUR_END = 20,
  BAR_COUNT = 15; // hours 6–20 inclusive
...
function buildSvg(byHour, allTotal = 0) {
  ...
  const totals = hours.map((h) => h.medical + h.admin + h.rxRoutine + h.rxNonRoutine + h.investigation);
  const maxTotal = Math.max(...totals, 0);

  if (maxTotal === 0) {
    // The chart only plots clinic hours (06:00–20:00) but "Total today" counts
    // all 24h, so a non-zero total with an empty chart is NOT a contradiction —
    // say which it is rather than flatly claiming "no submissions".
    const msg =
      allTotal > 0 ? `${allTotal} today, none within clinic hours (06:00–20:00)` : 'No submissions recorded today';
    return (
      `<svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block;">` +
      `<text x="${W / 2}" y="${H / 2}" text-anchor="middle" ` +
      `font-size="11" fill="var(--t3)">${msg}</text>` +
      '</svg>'
    );
  }
    ...
    .map((h, i) => {
      ...
      const rects = segments
        .map((seg) => {
          ...
          return (
            `<rect x="${x.toFixed(1)}" y="${y}" width="${w}" height="${pixH.toFixed(1)}" ` +
  ...
  return (
    `<svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block;" role="img" aria-label="Submission velocity histogram">` +
    gridLines +
    axes +
    bars +
    xLabels +
...
export function renderVelocity(data) {
  ...
  const svg = buildSvg(byHour, all);

  return (
    `<div class="condor-card condor-vel">` +
