...
// title:   accessible name for the chart (aria-label) — pass one per call-site
export function lineChart({ series, targets = [], bands = [], yMin, yMax, unit = '', title = '' }) {
  const W = 480,
    H = 240,
    PL = 40,
    PR = 14,
  ...
  if (!allVals.length)
    return '<svg viewBox="0 0 480 240" class="tc-svg"><text x="240" y="120" text-anchor="middle" class="tc-empty">No data</text></svg>';
  ...
  const axisTicks = yTicks
    .map((v) => {
      ...
      return (
        `<line x1="${PL}" y1="${yy}" x2="${W - PR}" y2="${yy}" class="tc-grid"/>` +
  ...
  const xLabels = allDates
    ...
    .map(
      (d) =>
        `<text x="${xScale(d).toFixed(1)}" y="${H - PB + 14}" class="tc-axis-lbl" text-anchor="middle">${esc(fmtDate(d))}</text>`
    )
    .join('');

  // Bands are dim reference washes; a 1px dashed line at each in-range upper
  // boundary keeps the zone edges legible without saturated fills.
  const bandRects = bands
    .map((b) => {
      const y1 = yScale(Math.min(b.hi, hi)).toFixed(1);
      const y2 = yScale(Math.max(b.lo, lo)).toFixed(1);
      const edge =
        b.hi > lo && b.hi < hi ? `<line x1="${PL}" y1="${y1}" x2="${W - PR}" y2="${y1}" class="tc-band-edge"/>` : '';
      return `<rect x="${PL}" y="${y1}" width="${W - PL - PR}" height="${(+y2 - +y1).toFixed(1)}" class="tc-band ${esc(b.cls)}"/>${edge}`;
    })
    .join('');
  ...
  const seriesEl = series
    .map((s) => {
      ...
      const d = pts
        .map((p) => `${pts.indexOf(p) ? 'L' : 'M'} ${xScale(p.date).toFixed(1)} ${yScale(p.value).toFixed(1)}`)
        .join(' ');
      const dots = pts
        .map(
          (p) =>
  ...
  return (
    `<svg viewBox="0 0 ${W} ${H}" class="tc-svg" role="img" aria-label="${esc(title || 'Trend chart')}">` +
