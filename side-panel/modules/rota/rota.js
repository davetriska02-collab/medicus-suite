...
// Medicus Suite — Rota status module (compact side-panel companion).
//
...
// reachable from the "Rota" nav tab or the button at the foot of this module.
//
...
function summarise(state) {
  ...
  try {
    ...
    // Malformed stored rota data must degrade to "no warnings shown", never
    // take the whole side panel down. The full app surfaces the real problem.
  }

  return { today, dates, duty, onLeave, vacancies, high, cap };
}
...
function renderStatus(s) {
  return `
    <div class="rota-module">
      ...
      ${renderVacancies(s.vacancies, s.today)}
      ${renderWarnings(s.high)}
  ...
  const row = (period, names) => {
    ...
    return `
      <div class="rota-duty-row">
        ...
        <span class="rota-pill ${gap ? 'rota-pill-red' : 'rota-pill-green'}">${gap ? 'Gap' : 'OK'}</span>
...
function renderVacancies(vacancies, today) {
  if (!vacancies.length) {
    return `
      <section class="rota-card">
