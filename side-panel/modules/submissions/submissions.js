...
const TASK_TYPES = [
  ...
  { key: 'investigation', label: 'Invest.',      shortLabel: 'Invest.',   type: 'review_investigation_results_task',     color: '#14b8a6' },
  { key: 'rxRoutine',     label: 'Routine Rx',   shortLabel: 'Rx rtn',    type: 'prescription_request_task_routine',     color: '#10b981' },
  { key: 'rxNonRoutine',  label: 'Non-rtn Rx',   shortLabel: 'Rx non',    type: 'prescription_request_task_non_routine', color: '#a855f7' },
];
...
function renderModeControls() {
  ...
  if (state.mode === 'today') {
    ...
    ctr.innerHTML =
      datePicker('From', state.rangeStart, v => { state.rangeStart = v; fetchAndRender(); }) +
      datePicker('To',   state.rangeEnd,   v => { state.rangeEnd = v; fetchAndRender(); }) +
    ...
    ctr.querySelectorAll('[data-days]').forEach(btn => {
      btn.addEventListener('click', () => {
        state.rangeEnd = todayISO(); state.rangeStart = addDays(todayISO(), -parseInt(btn.dataset.days));
        renderModeControls(); fetchAndRender();
      });
    });
  } else {
    ctr.innerHTML =
      datePicker('Day A', state.primaryDate, v => { state.primaryDate = v; fetchAndRender(); }) +
      datePicker('Day B', state.compareDate,  v => { state.compareDate = v; fetchAndRender(); }) +
      `<div class="preset-row">
    ...
    ctr.querySelector('[data-preset="yesterday"]')?.addEventListener('click', () => {
      state.primaryDate = todayISO(); state.compareDate = addDays(todayISO(), -1);
      renderModeControls(); fetchAndRender();
    });
    ctr.querySelector('[data-preset="lastweek"]')?.addEventListener('click', () => {
      state.primaryDate = todayISO(); state.compareDate = addDays(todayISO(), -7);
      renderModeControls(); fetchAndRender();
    });
  }
  // Bind date pickers
...
function buildHourlyCumulative(dayData) {
  ...
  for (const tt of TASK_TYPES) {
    ...
    let acc = 0; const cum = hourly.map(n => (acc += n));
...
function buildDailyTotals(rangeData, startISO, endISO) {
  const days = []; let cur = startISO;
  while (cur <= endISO) { days.push(cur); cur = addDays(cur, 1); }
  const byDay = {};
  for (const day of days) { byDay[day] = {}; for (const tt of TASK_TYPES) byDay[day][tt.key] = 0; }
  for (const tt of TASK_TYPES) {
...
function renderAll() {
  ...
  if (foot) foot.textContent = state.lastFetched
...
function renderToday() {
  const day = state.data.primary; if (!day) return;
  const series = buildHourlyCumulative(day);
  renderMetrics(TASK_TYPES.map(tt => ({ key: tt.key, label: tt.shortLabel, value: series[tt.key].total, color: tt.color })));
  const t = container.querySelector('#chart1Title'); if (t) t.textContent = 'Cumulative through the day';
  MWChart.line({ container: container.querySelector('#chart1'), xLabels: hourLabels(), series: TASK_TYPES.filter(tt => !state.hiddenSeries.has(tt.key)).map(tt => ({ key: tt.key, color: tt.color, label: tt.shortLabel, values: series[tt.key].cumulative })) });
...
function renderCompare() {
  const dayA = state.data.primary; const dayB = state.data.compare; if (!dayA || !dayB) return;
  const sA = buildHourlyCumulative(dayA); const sB = buildHourlyCumulative(dayB);
  renderMetrics(TASK_TYPES.map(tt => ({ key: tt.key, label: tt.shortLabel, value: sA[tt.key].total, compareValue: sB[tt.key].total, color: tt.color })));
  const t = container.querySelector('#chart1Title'); if (t) t.textContent = 'Cumulative · day vs day';
  const lineSeries = [];
  TASK_TYPES.forEach(tt => {
    if (state.hiddenSeries.has(tt.key)) return;
    lineSeries.push({ key: tt.key+'_a', color: tt.color, label: tt.shortLabel+' A', values: sA[tt.key].cumulative });
    lineSeries.push({ key: tt.key+'_b', color: tt.color, label: tt.shortLabel+' B', values: sB[tt.key].cumulative, dashed: true });
  });
...
function renderRange() {
  const data = state.data.primary; if (!data) return;
  const { days, byDay } = buildDailyTotals(data, state.rangeStart, state.rangeEnd);
  const totals = {}; for (const tt of TASK_TYPES) totals[tt.key] = days.reduce((a, d) => a + (byDay[d][tt.key]||0), 0);
  renderMetrics(TASK_TYPES.map(tt => ({ key: tt.key, label: tt.shortLabel, value: totals[tt.key], color: tt.color })));
  const t = container.querySelector('#chart1Title'); if (t) t.textContent = 'Daily submissions';
  const stacks = days.map(day => ({ day, segments: TASK_TYPES.filter(tt => !state.hiddenSeries.has(tt.key)).map(tt => ({ key: tt.key, value: byDay[day][tt.key]||0, color: tt.color, label: tt.shortLabel })) }));
...
function renderDelta(a, b) {
  ...
  let cls='flat', arrow='→';
  if (diff>0){cls='up';arrow='↑';} else if (diff<0){cls='dn';arrow='↓';}
  const text = b===0 ? `${arrow} ${a>0?`+${a} vs 0`:'no change'}` : `${arrow} ${diff>0?'+':''}${diff} (${pct>0?'+':''}${pct}%)`;
...
function renderLegend(elId, items, { toggleable=false, extraNote=null }={}) {
  ...
  el.innerHTML = items.map(it => `
    <div class="legend-item${state.hiddenSeries.has(it.key)?' muted':''}" data-key="${it.key}" style="cursor:${toggleable?'pointer':'default'}">
      ...
      <span>${it.label}</span>
    </div>`).join('') + (extraNote ? `<span class="legend-note">${extraNote}</span>` : '');
  if (toggleable) {
    el.querySelectorAll('.legend-item').forEach(li => {
      li.addEventListener('click', () => {
        const k = li.dataset.key;
        if (state.hiddenSeries.has(k)) state.hiddenSeries.delete(k); else state.hiddenSeries.add(k);
        renderAll();
      });
    });
  }
...
function showSkeleton() {
  const m = container?.querySelector('#subMetrics'); if (m) m.innerHTML = TASK_TYPES.map(tt => `<div class="sub-metric loading-skel"><div class="metric-label">${tt.shortLabel}</div><div class="metric-num zero">—</div></div>`).join('');
  const c1 = container?.querySelector('#chart1'); if (c1) MWChart.showLoading(c1, 'Fetching submissions…');
  const c2 = container?.querySelector('#chart2'); if (c2) MWChart.showLoading(c2, '');
  const l1 = container?.querySelector('#legend1'); if (l1) l1.innerHTML = '';
}

function setBanner(msg, kind='error') {
  const b = container?.querySelector('#subBanner'); if (!b) return;
  if (!msg) { b.classList.add('hidden'); return; }
  b.textContent = msg; b.className = 'banner' + (kind==='info'?' info':'');
}
...
const MWChart = {
  ...
  line({ container: el, xLabels, series }) {
    if (!el || !series.length) { MWChart.showLoading(el, 'No data'); return; }
    const W = el.clientWidth || 300, H = 120;
  ...
  bar({ container: el, bars }) {
    if (!el || !bars.length) { MWChart.showLoading(el, 'No data'); return; }
  ...
  stacked({ container: el, xLabels, stacks }) {
    if (!el || !stacks.length) { MWChart.showLoading(el, 'No data'); return; }
    const W = el.clientWidth || 300, H = 110;
    ...
    stacks.forEach((stack, i) => {
      ...
      stack.segments.forEach(seg => {
        ...
        yBase -= h;
        if (h > 0) rects += `<rect x="${x.toFixed(1)}" y="${yBase.toFixed(1)}" width="${barW.toFixed(1)}" height="${h.toFixed(1)}" fill="${seg.color}" opacity="0.85"/>`;
      });
      if (i % labelStep === 0) {
