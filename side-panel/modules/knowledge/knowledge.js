...
export async function init(el) {
  container = el;
  _query = ''; _activeCat = 'all'; _editingId = null; _expandedId = null;
  ...
  _storageListener = (changes, area) => {
    ...
    loadState().then(() => { if (container) render(); });
  };
...
function cleanup() {
  ...
  if (_similarTimer) { clearTimeout(_similarTimer); _similarTimer = null; }
  if (container) {
...
function renderCard(e) {
  ...
  const badges = [];
  if (e.source === 'llm' && !e.reviewed) badges.push('<span class="kb-badge kb-badge-amber">Unreviewed — AI-generated</span>');
  ...
  if (open) {
    ...
    const markReviewed = (e.source === 'llm' && !e.reviewed)
...
function renderForm() {
  ...
  const e = editing || { title: '', category: _activeCat !== 'all' ? _activeCat : (_categories[0]?.id || 'referrals'), body: '', phone: '', url: '', tags: [], reviewBy: '' };
  const catOpts = _categories.map(c =>
    `<option value="${esc(c.id)}" ${c.id === e.category ? 'selected' : ''}>${esc(c.name)}</option>`).join('');
  return `
    <div class="kb-form">
...
function renderSimilarPanel() {
  ...
  const hits = title.trim().length >= 3
    ? KU.findSimilar(title, _items, { excludeId: _editingId !== 'new' ? _editingId : null })
    : [];
  if (hits.length === 0) { host.innerHTML = ''; return; }
  host.innerHTML = `
...
async function saveForm() {
  ...
  let newCat = null;
  if (cat && typeof cat === 'object') { newCat = cat; cat = cat.id; }
  if (!cat) { errEl.textContent = 'Pick a category (or name the new one).'; return; }
  ...
  const raw = {
    ...
    tags: get('kbFmTags').split(',').map(s => s.trim()).filter(Boolean),
  ...
  if (errs.length > 0) { errEl.textContent = errs[0]; return; }
  ...
  if (_editingId === 'new') {
    ...
    if (idx === -1) { errEl.textContent = 'Entry no longer exists.'; return; }
