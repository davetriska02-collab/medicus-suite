...
| `board.html` / `board/` | Note — TV/monitor display board. `board/board-core.js` is pure (no DOM/`chrome.*`). Companion module is `side-panel/modules/board/`. Public profiles must never paint patient names (H-067). Live feeds/PPI still live in `side-panel/modules/condor/condor-data.js` and `condor-index-core.js` (the Condor tab itself was removed). |
...
> **Off-strip launchers (intentional exceptions):** `duplicate-checker` and `rota-app` are not strip buttons in either shell. They stay in the tab catalog (and the Practice manager preset, for Rota manager), and they are reached from the panel All-tabs menu under **Practice** and from the command palette (`open:rota` / `open:duplicates`) in both shells. `duplicate-checker` opens via `openDuplicateCheckerTab()` in `side-panel/duplicate-checker-open.js`; `rota-app` via `openRotaTab()` in `side-panel/modules/rota/rota-open.js` (focus-or-create). The compact Rota module's **Open full rota** button uses that same opener. They are excluded from `MODULES` (so the boot guard can't restore into them) and from the digit / Ctrl/Cmd+Alt+Arrow cycle. All *real* modules must still appear in both shells — including `rota` (labelled "Rota").
...
Five permanent strips live in `side-panel/panel.html` outside `<main>`, polled independently by `panel.js`:
...
- `#paStrip` — open-patient custom flags from Patient Alerts (`pa-strip-*` CSS)

Pattern: each strip has a hidden class, polls on load + interval, shows amber/red state when threshold crossed. If you add another global alert, follow this same pattern. These strips are panel-only by convention — `pop-out/pop-out.js`'s `MODULES` comment records this deliberately ("no WR/RM strips — they stay in the docked panel"); a new strip should match unless there's a specific reason to break the pattern.
