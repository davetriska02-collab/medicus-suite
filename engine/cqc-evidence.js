...
function deriveKeeperProvenance(drugFile) {
  ...
  else parts.push('Drug-monitoring rules curated from UK primary-care sources (BNF, NICE, BSR shared care, MHRA Drug Safety Update).');
  const tail = [];
...
function diffReadiness(current, anchor) {
  ...
  cmp('QOF safety-monitoring count', acov.qof && acov.qof.safetyMonitoringCount, cov.qof && cov.qof.safetyMonitoringCount);
  ...
  cmp('Vaccine rules last reviewed', acov.vaccine && acov.vaccine.lastUpdated, cov.vaccine && cov.vaccine.lastUpdated);
...
async function assembleReadiness(opts) {
  ...
  const currencyInput = [
    { id: 'drug', lastUpdated: ruleFiles.drug && ruleFiles.drug.lastUpdated, specVersion: ruleFiles.drug && ruleFiles.drug.specVersion },
    { id: 'qof', lastUpdated: ruleFiles.qof && (ruleFiles.qof.lastUpdated || ruleFiles.qof.version), specVersion: ruleFiles.qof && ruleFiles.qof.specVersion },
    { id: 'vaccine', lastUpdated: ruleFiles.vaccine && ruleFiles.vaccine.lastUpdated, specVersion: ruleFiles.vaccine && ruleFiles.vaccine.specVersion },
    { id: 'alert', lastUpdated: ruleFiles.alert && ruleFiles.alert.lastUpdated, specVersion: ruleFiles.alert && ruleFiles.alert.specVersion },
  ];
...
// ── Dual export (browser global + Node test harness) ─────────────────────────
  ...
  const api = { buildReadiness, diffReadiness, loadRuleFiles, assembleReadiness };
