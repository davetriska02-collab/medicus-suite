...
vaxRules.rules.forEach((rule) => {
  ...
  check(Array.isArray(given) && given.length > 0, `${rule.id}: statusTerms.given is a non-empty array`);
...
function checkEventCountRule(rule, source, libId) {
  ...
  if (rule.windowMonths != null) {
    check(
      Number.isFinite(rule.windowMonths) && rule.windowMonths > 0,
...
function checkBundleRule(rule, source, libId) {
  ...
  check(
    Array.isArray(rule.check.observations) && rule.check.observations.length > 0,
...
// ── medication-all-of checks: non-empty groups with non-empty match arrays ───
// An empty groups array is vacuously "achieved" — a clinical safety hazard.
console.log('\n--- medication-all-of checks: non-empty groups ---');
function checkMedAllOfRule(rule, source, libId) {
  if (rule.check?.kind !== 'medication-all-of') return;
  const rid = rule.id ?? libId ?? '?';
  if (rule.enabled === false) {
    console.log(`  INFO  ${rid} (${source}): medication-all-of disabled — skipping`);
    return;
  }
  const groups = rule.check.groups;
  check(Array.isArray(groups) && groups.length > 0, `${rid} (${source}): medication-all-of has non-empty groups array`);
  if (Array.isArray(groups)) {
    groups.forEach((g, i) => {
      check(
        Array.isArray(g.match) && g.match.length > 0,
        `${rid} (${source}): group[${i}] (${g.name || '?'}) has non-empty match array`
      );
    });
  }
}
(drugRules.rules || []).forEach((r) => checkMedAllOfRule(r, 'drug-rules.json'));
(qofRules.rules || []).forEach((r) => checkMedAllOfRule(r, 'qof-rules.json'));
(vaxRules.rules || []).forEach((r) => checkMedAllOfRule(r, 'vaccine-rules.json'));
(alertLib.library || []).forEach((e) => checkMedAllOfRule(e.rule || {}, 'alert-library.json', e.libId));
...
[
  ...
  check(
    typeof lu === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(lu) && !isNaN(Date.parse(lu)),
