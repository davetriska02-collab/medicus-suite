...
p(`**Tests:** ${meta.tests_passed ? '✅ passing' : '❌ FAILING — do not merge'}${(meta.tests_run && meta.tests_run.length) ? ` (${meta.tests_run.join(', ')})` : ''}`);
p('');
p('> **How to read this.** The Keeper compares the four Sentinel rule files against their ' +
...
if (weakening.length === 0) {
  ...
  p('These changes could *reduce* what Sentinel flags (longer interval, removed test, narrower ' +
  ...
  for (const c of weakening) {
    p(`| \`${esc(c.rule_id)}\` (${esc(c.domain)}) | ${esc(c.proposed)} | ${esc((c.provenance || {}).evidence || c.rationale)} | ${RAG_BADGE[c.rag] || c.rag} |`);
  }
}
p('');
...
for (const sec of SECTIONS) {
  ...
  for (const c of items) {
    ...
    p('');
    p(`- **Rule:** \`${esc(c.rule_id)}\`${c.needs_engine_change ? '  ⚙️ _needs rules-engine extension — ship disabled with placeholder_' : ''}`);
    p(`- **Now:** ${esc(c.current)}`);
    ...
    p(`- **Provenance:** verified by ${esc(pv.verified_by || 'unconfirmed')} on ${fmtDate(pv.checked_at)} — ${esc(pv.method || 'method not recorded')}, confidence ${esc(pv.confidence || 'unknown')}.`);
    p('');
  }
}
...
if (meta.source_gaps && meta.source_gaps.length) {
  p(`**⚠️ Sources that could not be reached this run:** ${meta.source_gaps.join('; ')}. _Treat the affected rules as unchecked this run._`);
  p('');
}
p('**Out of scope:** local ICB formularies and shared-care boundaries are not covered by this ' +
  'national scan. Paste a local formulary line into a run to fold it in.');
p('');
p('**Disclaimer:** The Keeper keeps Sentinel\'s approximations of the source guidance current. It is ' +
...
console.log(`Wrote ${outPath} (${applied} changes, ${reds.length} red, ${weakening.length} needing sign-off, ${killed.length} killed)`);
