...
// Single fixed clock for the whole file: every mock passes `now: NOW`, so obs/
// problem dates must be computed from the same instant — anchoring on the real
// Date() made the "overdue" boundary assertions rot as calendar time moved on.
...
function parseMock({ meds = [], obs = [], problems = [], age = null, sex = null, now }) {
  ...
  const observations = obs.map((l) => {
    const [name, value, date] = cols(l);
    return { name, code: null, value, date: date || null };
  });
  return {
    ...
    problems: problems.map((l) => {
      const [label, codedDate] = cols(l);
      return { label, codedDate: codedDate || null, status: 'active' };
    }),
...
function run(rule, mock, extra) {
  return engine.evaluatePatient(mock.medications, mock.observations, [...(extra || []), rule], {
    ...
    patientContext: mock.patientContext,
    observationHistory: mock.observationHistory,
  });
}
function fires(rule, mock, extra) {
  return run(rule, mock, extra).some((c) => c.ruleId === rule.id);
}
...
let threw = null;
try {
  validateCustomRule({ ...formRule, tests: [] });
} catch (e) {
  threw = e.message;
}
check(
  /tests must be a non-empty array/.test(threw || ''),
  'validator rejects a rule with no tests (live-preview ⚠ message)'
);

console.log('\n--- engine preview parity (overdue) ---');
let mock = parseMock({
  meds: ['Methotrexate 10mg tablets'],
  obs: [`FBC | normal | ${isoDaysAgo(84 + 60)}`],
  age: 60,
  now: NOW,
});
...
check(
  fired && fired.status === 'overdue',
  `status is overdue when FBC is ${84 + 60}d old (got ${fired && fired.status})`
);
...
check(
  run(formRule, mock).find((c) => c.ruleId === formRule.id) === undefined,
...
const dcRule = {
  id: 'custom-dc',
  type: 'drug-combo',
  label: 'NSAID + anticoagulant',
  drugSets: [
    { name: 'NSAID', match: ['ibuprofen'] },
    { name: 'Anticoagulant', match: ['warfarin'] },
  ],
  severity: 'red',
};
ok = true;
try {
  validateCustomRule(dcRule);
} catch (e) {
  ok = false;
  console.error('   ' + e.message);
}
...
check(
  fires(dcRule, parseMock({ meds: ['Ibuprofen 400mg', 'Warfarin 3mg'], age: 70, now: NOW })),
  'fires when both drug sets present'
);
check(
  !fires(dcRule, parseMock({ meds: ['Ibuprofen 400mg'], age: 70, now: NOW })),
  'does not fire with only one drug set'
);
...
ok = true;
try {
  validateCustomRule(ecRule);
} catch (e) {
  ok = false;
  console.error('   ' + e.message);
}
check(ok, 'event-count form object validates');
const utis = parseMock({
  problems: [
    `urinary tract infection | ${isoDaysAgo(20)}`,
    `urinary tract infection | ${isoDaysAgo(120)}`,
    `urinary tract infection | ${isoDaysAgo(200)}`,
  ],
  now: NOW,
});
check(fires(ecRule, utis), 'fires at 3 events within the window (>=3)');
check(
  !fires(ecRule, parseMock({ problems: [`urinary tract infection | ${isoDaysAgo(20)}`], now: NOW })),
  'does not fire at 1 event'
);
...
check(
  fires(ciRule, parseMock({ obs: [`HbA1c | 64 | ${isoDaysAgo(30)}`], age: 60, now: NOW })),
  'fires when HbA1c 64 crosses the >=58 threshold'
);
...
const cmRule = {
  id: 'custom-cm',
  type: 'composite',
  label: 'NSAID+AC AND recurrent UTI',
  operator: 'AND',
  ruleIds: ['custom-dc', 'custom-ec'],
  severity: 'red',
};
ok = true;
try {
  validateCustomRule(cmRule);
} catch (e) {
  ok = false;
  console.error('   ' + e.message);
}
...
check(
  fires(cmRule, bothMock, [dcRule, ecRule]),
  'AND-composite fires when both children fire (extraRules wired like cmExtraRules)'
);
...
check(
  alertChips.filter((c) => c.ruleId === alertRule.id).length === 0,
...
check(
  alertChipsValid.filter((c) => c.ruleId === alertRule.id).length === 1,
