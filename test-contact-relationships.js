...
console.log('14: isLikelyDuplicatePhone / findDuplicatePhoneGroups / choosePhoneToKeep');
{
  ...
  check(
    ...
    'a country-code form still matches the area-code-dropped short number'
  );
  check(
    CR.isLikelyDuplicatePhone('+44 20 8977 5481', '020 8977 5481') === true,
    '+44 replaces the leading 0 — national and international forms of the SAME number must match (the pair a raw suffix check misses)'
  );
  check(
    CR.isLikelyDuplicatePhone('0044 20 8977 5481', '020 8977 5481') === true,
    'an 00-prefixed international form is the same number as the national form'
  );
  check(
    CR.isLikelyDuplicatePhone('+44 7700 900123', '07700 900123') === true,
    'a +44 mobile matches the same 07 national mobile'
  );
  check(
    CR.isLikelyDuplicatePhone('+33 1 8977 5481', '020 8977 5481') === false,
    'a different-country number is never a duplicate of a UK national number just because the subscriber digits line up'
  );
  check(
    CR.canonicalUkPhoneDigits('+44 20 8977 5481') === '02089775481',
    'canonicalUkPhoneDigits rewrites +44… to the 0-prefixed national form'
  );
  check(
...
// ============================================================
// 16 — preferredFlagNeedsTransfer
// ============================================================
console.log('16: preferredFlagNeedsTransfer');
{
  const phones = [
    { telephoneNumber: '8977 5481', preferredTelephoneNumberForSms: true },
    { telephoneNumber: '020 8977 5481', preferredTelephoneNumberForSms: false },
  ];
  check(
    CR.preferredFlagNeedsTransfer(phones, 1, 'preferredTelephoneNumberForSms') === true,
    'keeping the fuller non-preferred number while deleting the SMS-preferred one needs a transfer'
  );
  check(
    CR.preferredFlagNeedsTransfer(phones, 0, 'preferredTelephoneNumberForSms') === false,
    'keeping the already-preferred number needs no transfer'
  );
  check(
    CR.preferredFlagNeedsTransfer(
      [{ preferredTelephoneNumberForSms: false }, { preferredTelephoneNumberForSms: false }],
      0,
      'preferredTelephoneNumberForSms'
    ) === false,
    'neither preferred -> no transfer'
  );
  check(CR.preferredFlagNeedsTransfer([], 0, 'preferredTelephoneNumberForSms') === false, 'empty list is a safe no-op');
  check(CR.preferredFlagNeedsTransfer(null, 0, 'preferredTelephoneNumberForSms') === false, 'null is a safe no-op');
}

// ============================================================
// 17 — unfinishedMergeConversionPlan / unfinishedMergeLeavePrompt
// ============================================================
console.log('17: unfinishedMergeConversionPlan / unfinishedMergeLeavePrompt');
{
  const asOther = CR.unfinishedMergeConversionPlan({
    existingForwardLink: null,
    baseId: 'wife',
    modifierId: 'ex',
    reverseBaseId: 'husband',
    reciprocalNeedsRepair: false,
  });
  check(asOther.mode === 'link-as-other', 'no existing link -> link-as-other');
  check(
    asOther.baseId === 'other' && asOther.reverseBaseId === 'other',
    'forces Other both ways, ignoring wife/husband guesses'
  );
  check(asOther.modifierId === null, 'clears any guessed modifier');
  check(asOther.relationshipUpdateId === null, 'never rewrites an existing relationship in this mode');

  const deleteOnly = CR.unfinishedMergeConversionPlan({
    existingForwardLink: { patientContactId: 'pc1', patientContactPatientId: 'cand-1' },
    baseId: 'daughter',
    reciprocalNeedsRepair: true,
    reciprocalRelationshipId: 'rec-1',
  });
  check(deleteOnly.mode === 'delete-manual-only', 'an existing forward link -> delete the manual duplicate only');
  check(
    deleteOnly.baseId === null && deleteOnly.reverseBaseId === null,
    'does not recategorise the existing Daughter link as Other'
  );
  check(deleteOnly.reciprocalUpdateId === null, 'does not repair/overwrite the reciprocal either');

  const repair = CR.unfinishedMergeConversionPlan({
    existingForwardLink: null,
    reciprocalNeedsRepair: true,
    reciprocalRelationshipId: 'rec-9',
  });
  check(
    repair.mode === 'link-as-other' && repair.reciprocalUpdateId === 'rec-9' && repair.reverseBaseId === 'other',
    'a placeholder reciprocal on a NEW link is updated in place to Other, not duplicated'
  );

  const leaveReverse = CR.unfinishedMergeConversionPlan({
    existingForwardLink: null,
    existingReciprocal: { patientContactRelationship: 'Mother' },
    reciprocalNeedsRepair: false,
  });
  check(
    leaveReverse.mode === 'link-as-other' && leaveReverse.reverseBaseId === null,
    'an already-real reverse link is left alone — never a second POST link-patient'
  );

  check(CR.unfinishedMergeLeavePrompt('close').includes('Click OK to close'), 'close path names close');
  check(
    CR.unfinishedMergeLeavePrompt('reload').includes('Click OK to reload the page'),
    'refresh path does not say "close anyway"'
  );
  check(CR.unfinishedMergeLeavePrompt('import').includes('Click OK to switch to Import'), 'import path names Import');
  check(
    CR.unfinishedMergeLeavePrompt('navigate').includes('Click OK to leave this patient'),
    'family-cycle path names leaving the patient'
  );
  check(
    !CR.unfinishedMergeLeavePrompt('reload').includes('close anyway'),
    'no leftover close-only wording on the reload prompt'
  );
}
