...
// Medicus Suite — workflow / inbound-document allocation canvas
//
// Sibling of the lab allocation canvas. Same drag / stage / confirm /
// bulk-reassign pattern, on document and workflow task-lists. The large
// left box is UNALLOCATED work, grouped by registered GP when that is
// on the row. Named GP is a grouping caption, never auto-placement.
// Does not file a document or mark a task complete.
//
...
(function () {
  ...
  if (window.__msWorkflowAllocateCanvas) return;
  window.__msWorkflowAllocateCanvas = true;

  var C = window.WorkflowAllocateCore;
  if (!C) return;

  var OVERLAY_ID = 'ms-wac-overlay';
  var LAUNCH_ID = 'ms-wac-launch';
  ...
  function currentRoute() {
    return C.parseWorkflowQueueRoute(location.pathname, location.search);
  }

  function client() {
    if (!_route) throw new Error('workflow-allocate: no workflow-queue route');
  ...
  async function loadBoard() {
    ...
    try {
      ...
      _rows = (_rows || []).map(function (row) {
        return C.decorateWorkflowRow(row, _route && _route.kind);
      });
  ...
  function emptyPoolHtml() {
    return (
      ...
      '</svg>' +
      '<div class="ms-lac-empty-title">No unallocated tasks on this queue</div>' +
      '<div class="ms-lac-empty-sub">Inbox work appears here. Work already sitting with a clinician is in their field on the right.</div>' +
      '</div>'
    );
  }
  ...
  function boardHtml() {
    ...
    return (
      ...
      board.count +
      '</span>' +
      '<span class="ms-lac-col-meta">Click a task, or a clinician heading for the lot. Ctrl-click adds more</span>' +
      '</div>' +
      (body || emptyPoolHtml()) +
      '</div>' +
  ...
  function confirmBarHtml() {
    ...
    if (_confirmWrite) {
      ...
      return (
        '<div class="ms-lac-confirmbar ms-lac-confirmbar-warn">' +
        '<strong>Medicus will reassign these tasks.</strong> This changes who the task sits with — it does not file the document or mark it complete.' +
    ...
    return (
      ...
      (blockReason
        ...
        : '<strong>Planning board.</strong> Staged moves live on this canvas until you review and confirm. Writing changes who the task sits with — it does not file the document or mark it complete' +
  ...
  function shellHtml() {
    ...
    return (
      ...
      '<h2 class="ms-lac-title" id="ms-lac-title">Allocate incoming work</h2>' +
