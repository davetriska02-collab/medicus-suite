...
(function () {
  ...
  // isn't excusing this" without guessing at normalisation.
  const DEBUG = (() => {
    try {
  ...
  function effectiveScore(rs, profile) {
    ...
    if (!profile || profile.paramsOverrideLabFlags !== true) {
      ...
      const fileBlockers = LF.fileabilityBlockers(rs.report, rs.severity, resultRules, profile);
    ...
    const fileBlockers = LF.fileabilityBlockers(adj, severity, resultRules, profile);
  ...
  function renderWhitelistBox(commentedResults, matchedProfiles) {
    ...
    whitelistBox.appendChild(
      el('div', 'chlf-wl-intro', 'Recognise a comment below? Whitelist it — for everyone, on the profile(s) it belongs to.')
    );
    const checks = [];
  ...
  async function evaluateGate() {
    ...
    if (blockers.length) {
      const commentedResults = LF ? LF.unresolvedCommentedResults(eff.report, profile) : [];
