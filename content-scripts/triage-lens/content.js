...
/* ---- Urgency bracket chips (Phase 1 — detail page only) ---- */
/* Own top-level family (not .ch-chip) — see hud.css for the full rationale. */
/* Kept in parity here for PiP (the HUD's request panel, where these render, */
/* is shown in PiP; the popover-evidence styling only lives in hud.css).     */
.ch-bracket {
  display: inline-block;
  padding: 2px 7px;
  border-radius: 10px;
...
.ch-bracket-red   { background: #fbe6e7; color: #b8262e; border-color: rgba(184,38,46,0.25); }
.ch-bracket-amber { background: #fdf3e1; color: #b45309; border-color: rgba(184,119,26,0.25); }
.ch-bracket-green { background: #e8f4ec; color: #2f8a4a; border-color: rgba(47,138,74,0.25); }
.ch-bracket-info  { background: #e7eef7; color: #2a4d7a; border-color: rgba(42,77,122,0.20); }
...
`;
  ...
  const recompileRules = () => {
    ...
    COMPILED_BRACKETS = (CONFIG?.urgencyBrackets || []).map(compileRule).filter(Boolean);
  };
  ...
  // prefix, retired in v3.75.0 — it kept rendering for existing users). When a stored chip
  ...
  const mergeShippedDefaults = (cfg) => {
    ...
    // Un-stick chip labels frozen at a since-changed shipped default (see RETIRED_CHIP_LABELS).
    ...
    // bracket ids are prefixed 'bracket-' so collision with a rule/resultRule id
    // is not a realistic concern, and reusing one tombstone array matches the
    // existing precedent (rules and resultRules already share it).
  ...
  // ---- Urgency brackets (Phase 1 — detail page only) ----
  // Practice-defined, named CATEGORY tags matched against the SAME request
  // free-text the detail-page rules use (buildFieldsData(data, initialReq.text)
  // in runDetail) — request text only, no other fields. Deliberately NOT a
  // composite urgency score: each bracket is its own named, sourced signal,
  // shown side by side with the existing rule chips (TRIAGE-NORTHSTAR-2026-07-22.md
  // design principles — "the GP is the aggregation function"). Advisory,
  // escalate-only, evidence-visible, never suppresses or reorders anything.
  const matchBrackets = (requestText) => {
    const text = requestText || '';
    if (!text) return [];
    return COMPILED_BRACKETS.filter(b => window.TriageLensMatch.ruleMatchesText(b, text));
  };

  // Rank matched brackets the same red<amber<info<green severity order the
  // rule chips use (RULE_CHIP_SEVERITY, defined below) — purely a display
  // order, never a suppression: every matched bracket is always shown.
  const computeBracketChips = (requestText) => {
    const matched = matchBrackets(requestText);
    const sorted = [...matched].sort((a, b) =>
      (RULE_CHIP_SEVERITY[a.kind] ?? 9) - (RULE_CHIP_SEVERITY[b.kind] ?? 9));
    return sorted.map(b => ({ kind: b.kind, text: b.label, bracketId: b.id }));
  };
  ...
  // ---- Urgency bracket chip rendering (Phase 1 — detail page only) ----
  // Deliberately its own top-level chip family (`ch-bracket`, kind modifiers
  // `ch-bracket-red/amber/green/info`) rather than `ch-chip`, so an advisory
  // practice-defined category tag can never be visually mistaken for a
  // scored/graded rule chip. Always clickable — every rendered bracket chip
  // has already matched, so there's always evidence to show (showBracketMenu).
  const renderBracketChipHtml = (chip) => {
    return `<span class="ch-bracket ch-bracket-${escapeHtml(chip.kind)}" data-bracket-id="${escapeHtml(chip.bracketId)}" title="${escapeHtml(chip.text)}">${escapeHtml(chip.text)}</span>`;
  };
  ...
  // ---- Urgency bracket evidence popover (Phase 1 — detail page only) ----
  // Reuses the SAME evidence affordance queue rule-match chips use
  // (buildEvidenceEl, above — unchanged) so a bracket's match evidence can
  // never look or behave differently from an existing rule chip's. A bracket
  // never carries actions (the shape has none — see defaults.json
  // urgencyBrackets), so the popover is evidence-only: a header badge plus
  // the matched-text block. Uses the SAME activeActionMenu/closeActionMenu
  // singleton as every other popover in this file (one open at a time).
  const showBracketMenu = (anchor, bracket, previewText) => {
    closeActionMenu();
    if (!bracket) return;
    const r = anchor.getBoundingClientRect();
    const menu = document.createElement('div');
    ...
    menu.setAttribute('aria-label', `${bracket.label} — practice-defined urgency bracket`);
    menu.style.position = 'fixed';
    menu.style.top = (r.bottom + 4) + 'px';
    menu.style.left = Math.max(8, Math.min(r.left, window.innerWidth - 320)) + 'px';
    menu.style.zIndex = '2147483647';
    ...
    badge.className = 'ch-bracket ch-bracket-' + bracket.kind;
    badge.textContent = bracket.label;
    head.appendChild(badge);
    menu.appendChild(head);

    menu.appendChild(buildEvidenceEl(bracket, previewText));
  ...
  const renderHUD = (data, signals, requestSignals) => {
    ...
    const requestPanel = (rs) => {
      if (!rs || (!rs.chips?.length && !rs.snippet && !rs.bracketChips?.length)) return '';
      return `
        <div class="ch-request">
          ...
          ${rs.bracketChips?.length ? `<div class="ch-request-row">${rs.bracketChips.map(c => renderBracketChipHtml(c)).join('')}</div>` : ''}
    ...
    // Urgency bracket chip click handlers — open the evidence-only popover
    // (Phase 1 — detail page only). requestSignals.bracketRequestText is the
    // exact text computeBracketChips matched against, closure-captured here
    // so the popover recomputes evidence lazily on open (same lazy pattern
    // buildEvidenceEl already uses for queue rule-match chips).
    hud.querySelectorAll('[data-bracket-id]').forEach(el => {
      el.addEventListener('click', (e) => {
        e.preventDefault(); e.stopPropagation();
        const bracket = COMPILED_BRACKETS.find(b => b.id === el.dataset.bracketId);
        if (bracket) showBracketMenu(el, bracket, requestSignals?.bracketRequestText || '');
      });
    });
  ...
  const runDetail = () => {
    ...
    // Urgency brackets (Phase 1 — detail page only): SAME request free-text
    // the detail-page rules just matched against (detailFieldsData.request),
    // never a separate extraction — see the pinned matching decision.
    requestSignals.bracketRequestText = detailFieldsData.request;
    requestSignals.bracketChips = computeBracketChips(detailFieldsData.request);
  ...
  // to a coarse relative form ("3 months ago") when the user prefers it
  // (oirDateFormat). Used in badge text and the confirm dialog.
  const fmtOirRelative = (iso) => {
  ...
  const fmtOirDate = (iso) => {
    if (!iso) return '';
    if (PREF('oirDateFormat', 'absolute') === 'relative') return fmtOirRelative(iso);
    const m = /^(\d{4})-(\d{2})-(\d{2})/.exec(String(iso));
    if (!m) return String(iso);
  ...
  // Built-in defs + the user/practice test dictionary (CONFIG.oirTests), merged
  // once per match. mergeTestDefs is append-only on built-ins (see engine).
  const oirTestDefs = () => {
    const OM = window.SentinelOutstandingMatch;
    if (!OM) return null;
    const user = CONFIG && Array.isArray(CONFIG.oirTests) ? CONFIG.oirTests : [];
    return user.length ? OM.mergeTestDefs(OM.TEST_DEFS, user) : OM.TEST_DEFS;
  };
  // Match options assembled from user prefs — dictionary, confidence floor,
  // look-back ceiling. Passed to both matchOutstanding and enrichWithHistory.
  const oirMatchOpts = () => ({
    testDefs: oirTestDefs(),
    confidenceFloor: PREF('oirConfidenceFloor', 'default'),
    lookbackMonths: PREF('oirLookbackMonths', 0),
  });
  // Compact " · 18 ug/L LOW" suffix for a matched result, shown on the inline flag
  // when the user opts in (oirShowValueOnBadge).
  const oirValueStr = (v) => {
    if (!v || !v.matchedValue) return '';
    ...
    try {
      ...
      const entry = {
        ...
        items: verdicts.map((v) => ({
          name: v.name,
          key: v.key || null,
          date: v.elsewhereDate || null,
          value: v.matchedValue || null,
          unit: v.matchedUnit || null,
          abnormal: v.matchedAbnormal || null,
        })),
      };
      chrome.storage.local.get('triagelens.oir.auditLog', (r) => {
        const arr = Array.isArray(r['triagelens.oir.auditLog']) ? r['triagelens.oir.auditLog'] : [];
        arr.unshift(entry);
        chrome.storage.local.set({ 'triagelens.oir.auditLog': arr.slice(0, 200) });
      });
    } catch (e) {
  ...
  const readOutstandingRows = (card) => {
    ...
    card.querySelectorAll('.q-checkbox').forEach((box) => {
      ...
      rows.push({ box, label: label.replace(/\s+/g, ' ').trim(), labelEl });
    });
    return rows;
  };
  ...
  // real consequences (a chase-up disappears from the worklist). Results found
  // ELSEWHERE in the record (not in this report) are NEVER auto-ticked — the
  ...
  // Inject (idempotently) the per-row verdict badge, anchored to THAT row's own
  // label element so every row is flagged in place.
  // (Earlier the badge was appended to the shared list container and de-duped
  // card-wide, so only ONE badge ever existed — it collapsed to the bottom of the
  // card showing the last row's verdict. Anchoring per row + keying the de-dupe by
  // row index fixes that: each row gets its own inline flag.)
  // patientUuid (may be undefined) makes resulted-elsewhere badges clickable.
  const annotateOutstandingRow = (rowMeta, verdict, patientUuid) => {
    ...
    const date = verdict.elsewhereDate ? fmtOirDate(verdict.elsewhereDate) : '';
    ...
    const val = PREF('oirShowValueOnBadge', false) ? oirValueStr(verdict) : '';
    let badgeText;
    if (resulted) {
      badgeText = tentative ? '✓? resulted? (this report)' : '✓ resulted (this report)';
    } else if (elsewhere) {
      if (tentative) {
        badgeText = date ? `↩? possible result ${date}${val} · in record` : `↩? possible result${val} · in record`;
      } else {
        badgeText = date ? `↩ completed ${date}${val} · in record` : `↩ found in record${val}`;
      }
    } else {
    ...
    // Clickable elsewhere badge → open patient care record (no per-observation
    // deep link exists in the API, so the care record is the closest target).
  ...
  // The bulk "Tick off results found in record" action bar — one gated button that
  // ticks every CONFIDENT resulted-elsewhere row at once, after the clinician
  // reviews the full enumerated list in confirmBulkTickOff. Tentative finds are
  // flagged inline only (never bulk-ticked — verify and tick manually). Idempotent:
  ...
  const updateBulkBar = (card, foundVerdicts, rows, taskUuid) => {
    ...
    if (!bar) {
      ...
      bar.className = 'ch-oir-bulk';
      const b = document.createElement('button');
      b.type = 'button';
      b.className = 'ch-oir-bulk-btn';
      bar.appendChild(b);
      card.appendChild(bar);
    }
    const btn = bar.querySelector('.ch-oir-bulk-btn');
    const n = pending.length;
    btn.textContent = `Tick off ${n} result${n > 1 ? 's' : ''} found in record`;
    // onclick (not addEventListener) so re-application never stacks handlers.
    btn.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  // Apply the engine verdict to the live card: annotate every row inline, auto-tick
  // the confident report-covered rows (once per task), and surface results found
  // elsewhere in the record via inline flags + the gated bulk tick-off bar.
  // `history` (may be null) enables the resulted-elsewhere enrichment.
  const applyOutstandingMatch = (card, report, taskUuid, history) => {
    if (!PREF('oirEnabled', true)) return;
    ...
    const opts = oirMatchOpts();
    let verdicts = OutstandingMatch.matchOutstanding(requests, report, opts);
    if (Array.isArray(history) && history.length) {
      verdicts = OutstandingMatch.enrichWithHistory(verdicts, history, opts);
    }

    // sharedCount — for each resulted_elsewhere verdict, count how many OTHER
    // resulted_elsewhere verdicts share the same key AND elsewhereDate.
    ...
    verdicts.forEach((v, idx) => {
      if (rows[idx]) annotateOutstandingRow(rows[idx], v, patientUuid);
    });
    ...
    // tick-off; tentative finds stay inline-only and are ticked manually if verified.
    // Advisory-only mode (oirBulkTickOff=false) keeps the flags but never offers a
    // machine-initiated write — the bar is removed if present.
    if (PREF('oirBulkTickOff', true)) {
      const foundConfident = verdicts.filter(
        (v) => v.status === 'resulted_elsewhere' && v.confidence === 'confident' && rows[v.id]
      );
      updateBulkBar(card, foundConfident, rows, taskUuid);
    } else {
      const bar = card.querySelector('.ch-oir-bulk');
      if (bar) bar.remove();
    }
  };
  ...
  function selectResultChips(sev) {
    if (!sev) return [];
    const chips = [];
    // Compose the rule-chip display label, de-duplicating the analyte name.
    // The Medicus results queue already shows the analyte name first, and our chip
    // prefixes it again ("{name} — {rule}"). When the user's rule label ALREADY
    // names the analyte (e.g. analyte "B12", rule "B12 low") that prefix just
    // duplicates it ("B12 — B12 low"). So: if the rule label already contains the
    // analyte name as a whole token, show the rule label alone; otherwise keep the
    // "{name} — {rule}" form. This lets a clinician name rules naturally
    // ("B12 low", "Ferritin borderline") without duplication and without needing
    // globally-unique bare labels ("low1", "low2") — labels are free-form (keyed by id).
    // Nested (not module-scope) so the regex test-extraction of this function stays self-contained.
    const composeResultRuleLabel = (name, rule) => {
      const n = (name || '').trim();
      const r = (rule || '').trim();
      if (!r) return n;
      if (!n) return r;
      ...
      const redundant =
        !!nn &&
        (rr === nn || rr.startsWith(nn + ' ') || rr.endsWith(' ' + nn) || rr.includes(' ' + nn + ' '));
      return redundant ? r : n + ' — ' + r;
    };
    ...
    // noGrowth (calm) — mirror the review path: a custom rule's normalLabel (e.g.
    // "Negative", "Not detected") shows via the attributable chip; the default "No
    // growth" (cultures) keeps the generic chip. The label comes from the matched
    // rule's normalLabel (sev.noGrowthTop.label), NOT the generic system-chip text.
    if (sev.noGrowthCount > 0) {
      const normalLabel = sev.noGrowthTop && sev.noGrowthTop.label ? String(sev.noGrowthTop.label) : '';
      if (normalLabel && normalLabel !== 'No growth') {
        chips.push({ id: 'queue.resultNoGrowthRule', vars: { label: normalLabel } });
      } else {
        chips.push({ id: 'queue.resultNoGrowth', vars: { count: sev.noGrowthCount } });
      }
    }
    // Combo rules — a fired combo (e.g. sterile pyuria) shows its own attributable
    // label. Escalate-only/amber; appears after the calm noGrowth path, before meta.
    if (sev.comboCount > 0 && sev.comboTop && sev.comboTop.label) {
      chips.push({ id: 'queue.resultCombo', vars: { rule: String(sev.comboTop.label) } });
    }
  ...
  const computeQueueRowResult = async (taskUuid) => {
    ...
    const comboCfg = findSystemChip('queue.resultCombo');
    const anyEnabled = [
      urgentCfg, abnormalCfg, ruleUrgentCfg, ruleAbnormalCfg, misPriCfg, unmatchCfg,
      reviewCfg, reviewRuleCfg, noGrowthCfg, noGrowthRuleCfg, comboCfg
