...
`;
  ...
  // ---- Document-context interceptor ----
  // Page-world injected script (see page-world.js) — passively re-dispatches the
  // GP's already-loaded filed care-record entries as a 'ch-doc-entries'
  // CustomEvent so we can show the descriptive "Filed notes ×N" chip. It reads
  // nothing the page didn't already request, makes no network calls of its own,
  // and sends nothing anywhere.
  const injectDocContextInterceptor = () => { /* no-op: see page-world.js (MAIN-world content script) */ };

  // ---- Document-context state + chips ----
  // Ephemeral, in-memory only: the count of filed care-record entries against
  // the open document, used purely for the descriptive "Filed notes ×N" chip.
  // Never written to chrome.storage and never added to any shared/io backup.
  let _docCtx = { entries: [], at: 0 };
  ...
  // Async chip render — modelled on runMonitoringChip. Surfaces the descriptive
  // "Filed notes ×N" chip from the cheap JSON the page already loaded (the GP's
  // filed care-record entry count). Staleness token guard prevents
  // wrong-document/wrong-patient bleed.
  const runDocContextChips = async () => {
    ...
    if (entryCount === 0) return; // no filed notes → no chip
    ...
    log('doc-context chips', { entryCount, chips: chips.map(c => c.text) });
  };
