...
`;
  ...
  const SYS_CHIP_DEFAULTS = {
    'queue.child':         { enabled: true, label: 'Child ({age})',    kind: 'amber', actions: [] },
    'queue.elder':         { enabled: true, label: 'Elder ({age})',    kind: 'amber', actions: [] },
    'queue.taskAgeAmber':  { enabled: true, label: '{days}d',          kind: 'amber', actions: [] },
    'queue.taskAgeRed':    { enabled: true, label: '{days}d',          kind: 'red',   actions: [] },
    'queue.priority':      { enabled: true, label: '{priority}',       kind: 'red',   actions: [] },
    'queue.readTime2m':    { enabled: true, label: '2m read',          kind: 'info',  actions: [] },
    'queue.readTime5m':    { enabled: true, label: '5m read',          kind: 'info',  actions: [] },
    'queue.readTime10m':   { enabled: true, label: '10m+ read',        kind: 'info',  actions: [] },
    'detail.statusAwaiting':      { enabled: true, label: '{status}',           kind: 'amber', actions: [] },
    'detail.statusReplyReceived': { enabled: true, label: '{status}',           kind: 'red',   actions: [] },
    'detail.statusClosed':        { enabled: true, label: '{status}',           kind: 'green', actions: [] },
    'detail.statusOther':         { enabled: true, label: '{status}',           kind: 'info',  actions: [] },
    'detail.priority':            { enabled: true, label: '{priority}',         kind: 'red',   actions: [] },
    'detail.daysOpenInfo':        { enabled: true, label: '{days}d open',       kind: 'info',  actions: [] },
    'detail.daysOpenAmber':       { enabled: true, label: '{days}d open',       kind: 'amber', actions: [] },
    'detail.daysOpenRed':         { enabled: true, label: '{days}d open',       kind: 'red',   actions: [] },
    'detail.today':               { enabled: true, label: 'today',              kind: 'info',  actions: [] },
    'detail.proxy':               { enabled: true, label: 'via {relationship}', kind: 'info',  actions: [] },
    'detail.attachments':         { enabled: true, label: '{count} attach',     kind: 'info',  actions: [] },
    'detail.readTime':            { enabled: true, label: '{label}',            kind: 'info',  actions: [] },
    'record.age':                 { enabled: true, label: '{age}y',             kind: 'info',  actions: [] },
    'record.palliative':          { enabled: true, label: 'Palliative',         kind: 'red',   actions: [] },
    'record.riskToSelf':          { enabled: true, label: 'Risk to self',       kind: 'red',   actions: [] },
    'record.frailtyRed':          { enabled: true, label: 'Frailty x{count}',   kind: 'red',   actions: [] },
    'record.frailtyAmber':        { enabled: true, label: 'Frailty x{count}',   kind: 'amber', actions: [] },
    'record.recentAdmissionRed':  { enabled: true, label: 'Admit {days}d',      kind: 'red',   actions: [] },
    'record.recentAdmissionAmber':{ enabled: true, label: 'Admit {days}d',      kind: 'amber', actions: [] },
    'record.polypharmacyRed':     { enabled: true, label: 'Meds x{count}',      kind: 'red',   actions: [] },
    'record.polypharmacyAmber':   { enabled: true, label: 'Meds x{count}',      kind: 'amber', actions: [] }
  };
  ...
  const estimateReadTime = (text) => {
    ...
    if (words < 30)  return { mins: 1,  label: '<1m read' };
    if (words < 100) return { mins: 2,  label: '2m read' };
    if (words < 200) return { mins: 5,  label: '5m read' };
    return { mins: 10, label: '10m+ read' };
  };
  ...
  const computeRequestSignals = (taskDetails, requester, initialReq) => {
    ...
    // Read time
    const rt = estimateReadTime(ir.text);
    if (rt) pushOut('detail.readTime', { label: rt.label });
  ...
  const runDetail = () => {
    ...
    const requester = extractRequester();
    const initialReq = extractInitialRequest();
    ...
    restorePosition(hudEl);
    enableDrag(hudEl);
    log('detail rendered', { data, signals, taskDetails, requester, initialReq });
  };

  const runQueue = () => {
    // Tear down any previous observer so we always re-attach to the current
    // container node — AG Grid may have been destroyed and rebuilt by the SPA.
  ...
  const decorateOneRow = (row) => {
    ...
    // Read time — pick the right system chip based on length
    const rt = estimateReadTime(previewText);
    if (rt) {
      if (rt.mins >= 10)      pushSysChip('queue.readTime10m');
      else if (rt.mins >= 5)  pushSysChip('queue.readTime5m');
      else if (rt.mins >= 2)  pushSysChip('queue.readTime2m');
      // mins == 1 → no chip (skip the noise of "<1m" on every short request)
    }
