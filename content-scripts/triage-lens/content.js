...
`;
  ...
  function selectResultChips(sev) {
    ...
    if (sev.level === 'red' && sev.urgentCount > 0) {
      chips.push({ id: 'queue.resultUrgent', vars: { name: sev.top ? sev.top.name : '', count: sev.urgentCount } });
    ...
    if (sev.reviewCount > 0) chips.push({ id: 'queue.resultReview', vars: { count: sev.reviewCount } });
  ...
  const computeQueueRowResult = async (taskUuid) => {
    ...
    const anyEnabled = [urgentCfg, abnormalCfg, misPriCfg, unmatchCfg].some(c => c && c.enabled !== false);
    ...
    return SEV.evaluateReportSeverity(report, {
      ...
      resultRules: (CONFIG && Array.isArray(CONFIG.resultRules)) ? CONFIG.resultRules.filter(r => r && r.enabled !== false) : []
    });
  };
