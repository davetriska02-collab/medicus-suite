...
`;
  ...
  const refreshPulseOnRow = (row) => {
    ...
    if (open) host.classList.add('ch-q-pulse-open');
    ...
    if (open === 'why') {
      const whyTray = buildPulseWhyTray(composed);
      whyTray.id = whyTrayId;
      host.appendChild(whyTray);
    }
    if (open === 'act') {
      const actTray = buildPulseActTray(row, act);
      actTray.id = actTrayId;
      host.appendChild(actTray);
    }
