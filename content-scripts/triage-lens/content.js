...
`;
  ...
  // 1. DATE HELPERS
  // ============================================================
  const _dateHelpers = globalThis.TriageDateHelpers;
  if (!_dateHelpers) {
    throw new Error('[ClinHUD] TriageDateHelpers missing — load date-helpers.js before content.js');
  }
  const MONTHS = _dateHelpers.MONTHS;
  const NOW = _dateHelpers.NOW;
  const parseDate = _dateHelpers.parseDate;
  const monthsAgo = _dateHelpers.monthsAgo;
  const daysAgo = _dateHelpers.daysAgo;
