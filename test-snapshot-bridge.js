...
check(
  !/SentinelRules\.evaluatePatient\s*=/.test(sentinel),
...
check(
  lastSnapshotAssignments === 2,
  `_lastSnapshot is re-assigned in exactly 2 places (found ${lastSnapshotAssignments}: publishSnapshot + invalidateSnapshot)`
);

check(/function publishSnapshot\(/.test(sentinel), 'publishSnapshot() exists as the single good-data writer');

check(
  /window\.__msReadSentinelSnapshot\s*=/.test(sentinel),
  "exposes window.__msReadSentinelSnapshot for same-page readers (floating What's due)"
);
check(/ms-sentinel-snapshot/.test(sentinel), 'notifySnapshotUpdated pings same-page readers via ms-sentinel-snapshot');
check(!/dispatchEvent\([^)]*chips/.test(sentinel), 'the same-page ping does not put chips on the DOM event');

check(
  /publishSnapshot\(\s*chips\s*,\s*data\.patientContext/.test(sentinel),
...
check(
  /engine\s*=\s*window\.SentinelRules/.test(triage),
  'triage-lens content.js uses window.SentinelRules (shared engine global)'
);
check(
  /rules\/drug-rules\.json/.test(triage) && !/qof-rules\.json/.test(triage),
...
check(/_evalGen/.test(sentinel), 'evaluation generation counter (_evalGen) present');
check(/const gen = \+\+_evalGen/.test(sentinel), 'evaluateAndPublish captures a fresh generation before fetching');
check(
  /if \(gen !== _evalGen\) return/.test(sentinel),
...
check(
  /function evaluateAndPublish/.test(sentinel) && /fetchJournalObservations\(\s*_patientId/.test(sentinel),
...
check(
  /data\.patientContext\?\.patientUuid/.test(sentinel),
