...
if (!_CT) throw new Error('ClinicalThresholds missing — load shared/clinical-thresholds.js before visualiser-core.js');
const RCV_TABLE = _CT.RCV_TABLE;
const CLINICAL_ZONES = _CT.CLINICAL_ZONES;
...
const QOF_REGISTERS = [
  { id: 'hf', label: 'Heart Failure', terms: ['heart failure'], reviewMonths: 12 },
  {
    ...
    terms: ['atrial fibrillation', 'persistent atrial', 'paroxysmal atrial'],
    reviewMonths: 12,
  },
  ...
  { id: 'dem', label: 'Dementia', terms: ['dementia', 'alzheimer'], reviewMonths: 12 },
  {
    ...
    terms: ['cancer', 'malignancy', 'carcinoma', 'tumour', 'lymphoma', 'leukaemia'],
    reviewMonths: 6,
  },
  ...
  { id: 'epilepsy', label: 'Epilepsy', terms: ['epilepsy', 'epileptic'], reviewMonths: 12 },
  {
    id: 'stroke',
    label: 'Stroke / TIA',
    terms: ['stroke', 'transient ischaem', 'tia', 'cerebrovascular'],
    reviewMonths: 12,
  },
  {
    id: 'chd',
    label: 'Coronary Heart Disease',
    terms: ['coronary heart', 'ischaemic heart', 'angina', 'myocardial infarction', 'heart disease'],
    reviewMonths: 12,
  },
...
const CHARLSON_WEIGHTS = [
  ...
  },
  {
    ...
    weight: 1,
    terms: ['stroke', ' tia ', 'transient ischaem', 'cerebrovascular'],
  },
  ...
  { id: 'copd', label: 'COPD', weight: 1, terms: ['copd', 'chronic obstructive', 'emphysema'] },
  {
    ...
    weight: 1,
    terms: ['rheumatoid arthritis', 'systemic lupus', 'connective tissue'],
  },
  ...
  { id: 'liver_mild', label: 'Mild liver disease', weight: 1, terms: ['chronic hepatitis', 'cirrhosis'] },
  {
    ...
    label: 'Diabetes',
    weight: 1,
    terms: ['diabetes mellitus', 'type 1 diabetes', 'type 2 diabetes', 'diabetic'],
  },
  { id: 'hemiplegia', label: 'Hemiplegia / paraplegia', weight: 2, terms: ['hemiplegia', 'paraplegia'] },
  {
    ...
    label: 'Renal disease',
    weight: 2,
    terms: ['chronic kidney disease', 'ckd stage', 'end stage renal', 'dialysis'],
  },
  {
    id: 'malignancy',
    label: 'Malignancy',
    weight: 2,
    terms: ['cancer', 'malignancy', 'carcinoma', 'lymphoma', 'leukaemia'],
  },
  {
    id: 'metastatic',
    label: 'Metastatic solid tumour',
    weight: 6,
    terms: ['metastatic', 'metastases', 'secondary malignancy'],
  },
  {
    id: 'aids',
    label: 'AIDS / HIV',
    weight: 6,
    terms: [' aids ', 'aids-defining', 'hiv infection', 'hiv positive', 'hiv disease', 'human immunodeficiency'],
  },
];
...
const _PT = globalThis.PincerTables;
if (!_PT) throw new Error('PincerTables missing — load engine/pincer-tables.js before visualiser-core.js');
const HIGH_RISK_DRUGS = _PT.HIGH_RISK_DRUGS;

// PINCER-style drug-disease and monitoring-overdue rules. Run after both the
// drug list and the problem list are known.
// ageStr: _s.demographics?.age as a string (e.g. "72"); may be undefined/null.
// Age-gated flags fail-closed: if ageStr is missing/unparseable, the flag does NOT fire.
function computePINCER(allProblems, drugs, entries, ageStr) {
  const flags = [];
  ...
  // Parse age — fail-closed for age-gated flags
  const age = parseInt(ageStr, 10);
  const ageKnown = Number.isFinite(age);

  // Gastroprotection helper: PPI or H2-blocker present in detected drugs
  const hasGastroprotection = () => !!(drugFor('ppi') || drugFor('h2blocker'));

  // GI-bleed / peptic-ulcer history terms
  const GI_PROBLEM_TERMS = [
    'peptic ulcer',
    'gastric ulcer',
    'duodenal ulcer',
    'gi bleed',
  ...
  const hasGiHistory = () => GI_PROBLEM_TERMS.some((t) => has(t));

  // ── Existing flags (source field added) ──────────────────────────────────

  if (drugFor('nsaid_long') && has('chronic kidney')) {
    flags.push({
      severity: 'high',
      rule: 'NSAID prescribed with CKD',
      detail:
        'Long-term NSAID in chronic kidney disease — risk of further GFR decline. Review need and consider alternative.',
      source: 'PINCER / NICE',
    });
  }
  if (drugFor('nsaid_long') && has('heart failure')) {
    flags.push({
      severity: 'high',
      rule: 'NSAID with heart failure',
      detail:
        'NSAID can precipitate decompensation in heart failure. Avoid or use lowest effective dose for shortest duration.',
      source: 'PINCER #6 / NICE',
    });
  }
  if (drugFor('nsaid_long') && (drugFor('warfarin') || drugFor('doac'))) {
    flags.push({
      severity: 'high',
      rule: 'NSAID with oral anticoagulant',
      detail: 'Increased GI bleeding risk. Consider gastro-protection (PPI) and review need.',
      source: 'PINCER #2 / NICE',
    });
  }
  if (drugFor('beta_block') && has('asthma')) {
    flags.push({
      severity: 'high',
      rule: 'Beta-blocker with asthma',
      detail:
        'Beta-blockers (especially non-selective) may precipitate bronchospasm — review indication, prefer cardioselective if essential.',
      source: 'PINCER / BNF',
    });
  }

  // ── New flags P-A through P-F ─────────────────────────────────────────────

  // P-A: NSAID + GI bleed / peptic ulcer history, no gastroprotection
  if (drugFor('nsaid_long') && hasGiHistory() && !hasGastroprotection()) {
    flags.push({
      severity: 'high',
      rule: 'NSAID with peptic ulcer/GI bleed history — no gastroprotection',
      detail:
        'NSAID prescribed in a patient with history of peptic ulcer or GI bleed without co-prescribed PPI or H2-blocker. High GI haemorrhage risk.',
      source: 'PINCER / NICE',
    });
  }

  // P-B: Antiplatelet + GI bleed / peptic ulcer history, no gastroprotection
  if ((drugFor('antiplatelet') || drugFor('aspirin_ap')) && hasGiHistory() && !hasGastroprotection()) {
    flags.push({
      severity: 'high',
      rule: 'Antiplatelet with peptic ulcer/GI bleed history — no gastroprotection',
      detail:
        'Antiplatelet prescribed in a patient with history of peptic ulcer or GI bleed without co-prescribed PPI or H2-blocker.',
      source: 'PINCER / NICE',
    });
  }

  // P-C: NSAID without gastroprotection in age ≥65 (PINCER #1)
  // Fail-closed: if age unknown, this flag does NOT fire.
  if (drugFor('nsaid_long') && ageKnown && age >= 65 && !hasGastroprotection()) {
    flags.push({
      severity: 'high',
      rule: 'NSAID in age ≥65 without gastroprotection (PINCER #1)',
      detail:
        'NSAID prescribed in patient aged ≥65 without co-prescribed PPI or H2-blocker. Elevated GI bleeding risk in older adults.',
      source: 'PINCER #1 / NICE',
    });
  }

  // P-D: Oral anticoagulant + antiplatelet, no gastroprotection (PINCER #13 analogue)
  if (
    (drugFor('warfarin') || drugFor('doac')) &&
    (drugFor('antiplatelet') || drugFor('aspirin_ap')) &&
    !hasGastroprotection()
  ) {
    flags.push({
      severity: 'high',
      rule: 'Oral anticoagulant + antiplatelet without gastroprotection',
      detail:
        'Concurrent anticoagulant and antiplatelet without gastroprotection — significantly elevated GI bleed risk.',
      source: 'PINCER #13 / NICE',
    });
  }

  // P-E: Aspirin + second antiplatelet without PPI (dual antiplatelet, PINCER #8)
  if (drugFor('aspirin_ap') && drugFor('antiplatelet') && !drugFor('ppi')) {
    flags.push({
      severity: 'med',
      rule: 'Dual antiplatelet (aspirin + P2Y12) without PPI (PINCER #8)',
      detail:
        'NICE recommends PPI co-prescription with dual antiplatelet therapy. Review duration — usually time-limited post-ACS or PCI.',
      source: 'PINCER #8 / NICE',
    });
  }
  ...
  const hasLoopDiuretic = () => {
    const loop = drugs.find(
      (d) =>
        ...
        d.active &&
        d.terms &&
        d.terms.some((t) => ['furosemide', 'frusemide', 'bumetanide'].includes(t))
    );
    return !!loop;
  };
  if (ageKnown && age >= 75) {
    const aceiForPF = drugFor('acei');
    const aceiUeOverdue =
      aceiForPF &&
      aceiForPF.active &&
      (!aceiForPF.lastMonitoring || new Date() - aceiForPF.lastMonitoring > 456 * 864e5);
    if (aceiUeOverdue) {
      flags.push({
        severity: 'high',
        rule: 'ACEi/ARB in age ≥75 — U&E overdue (>15 months)',
        detail:
          'ACEi/ARB in patient aged ≥75 without U&E in the last 15 months. Increased hyperkalaemia and AKI risk in older adults.',
        source: 'PINCER / NICE',
      });
    }
    // Loop diuretic arm: detect loop diuretics from the diuretic group
    const diureticDrug = drugFor('diuretic');
    if (diureticDrug && diureticDrug.active) {
      const ueOverdueLong = !diureticDrug.lastMonitoring || new Date() - diureticDrug.lastMonitoring > 456 * 864e5;
      if (ueOverdueLong) {
        flags.push({
          severity: 'high',
          rule: 'Loop/thiazide diuretic in age ≥75 — U&E overdue (>15 months)',
          detail:
            'Diuretic in patient aged ≥75 without U&E in the last 15 months. Increased risk of electrolyte disturbance and AKI in older adults.',
          source: 'PINCER / NICE',
        });
      }
    }
  ...
  // LABA: combo-inhaler brand detection is error-prone without a complete UK brand list.
  // COCP: requires complete UK COCP brand enumeration (well over 20 brands).
  // OMITTED PERMANENTLY: oestrogen-only HRT with intact uterus — uterus status undetectable.

  // ── Monitoring-overdue flags (existing logic, source added) ──────────────

  for (const d of drugs) {
    if (d.overdue && d.lastSeen) {
      flags.push({
        severity: 'med',
        rule: `${d.label} monitoring overdue`,
        ...
        source: 'BNF / NICE monitoring guidance',
      });
    }
  }

  // ACEi without recent U&E AND has CKD = serious (existing, source added)
  const acei = drugFor('acei');
  if (acei && acei.overdue && has('chronic kidney')) {
    flags.push({
      severity: 'high',
      rule: 'ACEi/ARB + CKD — U&E overdue',
...
function practitionerColourMap(practitioners) {
  ...
  practitioners.forEach((p, i) => {
    m[p] = PRACTITIONER_PALETTE[i % PRACTITIONER_PALETTE.length];
  });
  return m;
}
...
function computeWhatsNew(entries) {
  const consults = entries.filter((e) => e.bucket === 'consultation' && e.date).sort((a, b) => b.date - a.date);
...
const ENTRY_HDR_RE =
  /^(.+?)\s*[•]\s*(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\s+(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{4})/i;
...
function parseAll(pageTexts) {
  ...
  while (i < allLines.length) {
    const l = allLines[i];
    if (l === 'Administrative & Contact Details') {
      i++;
      break;
    }
    i++;
  ...
  while (i < allLines.length) {
    ...
    if (/^Home phone/i.test(l) && !demographics.phone)
      demographics.phone = l.replace(/^Home phone[^:]*:\s*/i, '').trim();
  ...
  while (i < allLines.length) {
    ...
    if (inActive || inPast) {
      ...
      if (pm) {
        const prob = {
          name: pm[1].trim(),
          severity: pm[2].toLowerCase(),
          dateStr: pm[3].trim(),
          date: parseDateStr(pm[3]),
        };
    ...
    for (const bl of body) {
      ...
      if (/^Status:/i.test(bl))
        status = bl
          ...
          .split(/\s*•/)[0]
          .trim();
      if (/^Planned date:/i.test(bl))
        plannedDate = bl
          .replace(/^Planned date:\s*/i, '')
          .trim()
          .split(/\s*•/)[0]
          .trim();
      ...
      if (
        /\s+BLOOD$|\s+PROFILE$|FBC$|LFT$|BONE PROFILE$|BLOOD FILM$|U-S |Ultrasound|Serum |HbA1c|TSH|cholesterol|troponin/i.test(
          bl
        )
      ) {
...
function computeEFI(activeProblems, pastProblems, drugs) {
  ...
  if (score > 0.36) {
    ...
    category = 'Moderate frailty';
    colour = '#f47738';
  } else if (score > 0.12) {
    category = 'Mild frailty';
    colour = '#ffeb3b';
  } else {
    category = 'Fit';
    colour = '#007f3b';
  ...
  for (const d of HIGH_RISK_DRUGS) {
    ...
    if (d.requires.length) {
      const monMatches = invEntries.filter((e) => {
        const hay = (e.results || [])
          .map((r) => r.panel.toLowerCase() + ' ' + r.items.map((i) => i.name.toLowerCase()).join(' '))
          .join(' ');
...
function buildFilterBarBrush() {
  ...
  const brush = d3
    ...
    .on('end', (event) => {
      ...
      if (!event.selection) {
        _s.filter.dateFrom = null;
        _s.filter.dateTo = null;
        _s.filter.preset = 'all';
      } else {
        const [a, b] = event.selection.map(x.invert);
        _s.filter.dateFrom = a;
        _s.filter.dateTo = b;
        _s.filter.preset = 'custom';
      }
      document
        .querySelectorAll('.fb-preset')
        .forEach((b) => b.classList.toggle('active', b.dataset.preset === _s.filter.preset));
      rebuildAll();
    });
...
function wireFilterBar() {
  ...
  document.getElementById('fb-clear').addEventListener('click', () => {
    ...
    if (fb) {
      _s._fbSuppressEnd = true;
      try {
        fb.brushG.call(fb.brush.move, null);
      } finally {
        _s._fbSuppressEnd = false;
      }
    }
...
function rebuildAll() {
  ...
  const pincer = computePINCER([..._s.activeProblems, ..._s.pastProblems], drugs, _s.entries, _s.demographics?.age);
...
function buildSnapshot(d, entries, recalls, efi, pincer, drugs, charlson, registersWithReview) {
  ...
  const efiPct = Math.min(efi.score / 0.5, 1); // 0..1 of arc
      ...
      <div class="efi-detail">
        ...
        <div class="efi-meta">
          ...
          ${efi.ticked
            .slice(0, 4)
            .map((t) => esc(t.label))
            .join(' · ')}${efi.ticked.length > 4 ? ` · +${efi.ticked.length - 4} more` : ''}
        </div>
      </div>
    </div>
  </div>`;
  ...
  const condText = chItems.length
    ? chItems
        .slice(0, 4)
        .map((it) => `<span title="${esc(it.evidence)}">${esc(it.label)}</span>`)
        .join(' · ') + (chItems.length > 4 ? ` · +${chItems.length - 4} more` : '')
    ...
    const items = pincer
      ...
      <div class="pincer-item ${esc(f.severity)}">
        ...
        <div style="flex:1">
          ...
          ${f.source ? `<div class="pincer-source">${esc(f.source)}</div>` : ''}
        </div>
      </div>`
      )
      .join('');
  ...
  if (whatsNew && whatsNew.items.length) {
    const pivotStr = whatsNew.pivot.date.toLocaleDateString('en-GB', {
      day: 'numeric',
      month: 'short',
      year: 'numeric',
    });
    ...
    const moreNote =
      whatsNew.items.length > 12
        ? `<div style="font-size:11px;color:#768692;margin-top:6px">+ ${whatsNew.items.length - 12} more event(s) since last consultation.</div>`
        : '';
    const summaryChips = Object.entries(grouped)
      .map(
        ([b, arr]) =>
          `<span class="badge badge-grey" style="background:${BUCKET_COLOUR[b]}20;color:${BUCKET_COLOUR[b]};margin-right:6px">${arr.length} ${esc(b)}</span>`
      )
      .join('');
  ...
  if (openRecalls.length) {
    ...
    for (const r of openRecalls) {
      const badge =
        r.days === null
          ? `<span class="badge badge-grey">Date unknown</span>`
          : r.days < 0
            ? `<span class="badge badge-red">Overdue ${-r.days}d</span>`
            : r.days < 30
              ? `<span class="badge badge-amber">Due in ${r.days}d</span>`
...
function buildContinuity(clinicians, entries, upc, bice, contactableEntries) {
  ...
  if (consults.length >= 1) {
    ...
    const legendHtml = visiblePract
      ...
      .join('');
    const moreNote =
      visiblePract.length > 24
        ? `<span style="font-size:11px;color:#768692">+${visiblePract.length - 24} more</span>`
        : '';
  ...
  const filterNote =
    cliFilter || _s.filter.preset !== 'all'
  ...
  document
    .querySelectorAll('#ribbon-legend [data-clinician]')
...
function buildTimeline(entries) {
  // Legend + filter note
  const legendItems = Object.entries(BUCKET_COLOUR)
    .map(
      ([b, c]) =>
...
function renderContactsHeatmap(tl) {
  ...
  const legendSwatches = HEAT_BLUES.map(
    (c) => `<span style="width:14px;height:14px;border-radius:3px;background:${c};display:inline-block"></span>`
  ).join('');
...
function renderSwimLane(entries) {
  ...
  presentLanes.forEach((l, i) => {
    g.append('rect')
      .attr('class', i % 2 ? 'lane-bg alt' : 'lane-bg')
      .attr('x', 0)
      .attr('y', i * laneH)
      .attr('width', innerW)
      .attr('height', laneH);
  ...
  const xAxis = d3
    .axisBottom(x)
    .ticks(Math.min(10, Math.max(3, Math.round(innerW / 100))))
    .tickSizeOuter(0);
  ...
  eventG
    ...
    .on('mouseenter', (event, d) => {
      ...
      const lines = [
        ...
        d.code ? esc(d.code) : '',
        d.linkedProblems && d.linkedProblems.length
          ? `<em>Linked:</em> ${esc(d.linkedProblems.slice(0, 2).join(' · '))}`
          : '',
      ].filter(Boolean);
...
function renderInvLatestTable(analytes) {
  ...
  if (!rows)
    rows =
      '<tr><td colspan="7" style="text-align:center;color:#768692;padding:20px">No analytes match the filter.</td></tr>';
  body.innerHTML = rows;
  ...
  body.querySelectorAll('tr[data-analyte]').forEach((tr) => {
    tr.addEventListener('click', () => {
      ...
      if (sel) {
        sel.value = an;
        renderAnalyteTrend(analytes, an);
      }
...
function renderAnalyteTrend(analytes, name) {
  ...
  const labels = pts.map((p) =>
    p.date ? p.date.toLocaleDateString('en-GB', { month: 'short', year: 'numeric' }) : '?'
  );
      ...
      datasets: [
        {
          ...
          data: vals,
          borderColor: '#005eb8',
          backgroundColor: 'rgba(0,94,184,.08)',
          borderWidth: 2,
          pointRadius: 4,
          tension: 0.2,
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ticks: { font: { size: 11 } },
          beginAtZero: false,
          suggestedMin: vMin - padding,
          suggestedMax: vMax + padding,
        },
      },
    },
...
function buildMedications(drugs, pincer) {
  ...
  const tile = (n, l, c) =>
    `<div class="stat-tile"><div class="stat-num" style="color:${c || 'var(--nhs-blue)'}">${n}</div><div class="stat-lbl">${esc(l)}</div></div>`;
  ...
  if (active.length) {
    for (const d of active) {
      ...
      const monStr =
        d.requires.length === 0
          ? '<span style="color:#b1b4b6">N/A</span>'
          : d.lastMonitoring
            ? fmt(d.lastMonitoring) + ` <span class="drug-meta">(${d.daysSinceMonitoring}d ago)</span>`
            : '<span style="color:var(--nhs-red);font-weight:600">No record</span>';
      const badge =
        d.requires.length === 0
          ? `<span class="badge badge-grey">No monitor</span>`
          : d.overdue
  ...
  <div class="card" style="background:#fff9e6;border-color:#ffe082">
    ...
    <div style="font-size:12px;color:var(--text2);line-height:1.5">
      Drugs are detected by scanning entry text for the drug name. The visualiser cannot tell whether a prescription is current, stopped, or one-off — "active" means the drug has been mentioned in the last 18 months. PINCER-style flags compare detected drugs against problem-list conditions and monitoring intervals from BNF / NICE. Use this as a screen, not a definitive medication review. Gastroprotection detection is limited to common PPIs and H2-blockers mentioned in the visible record — absence of a PPI or H2-blocker in the detected list may simply mean it was not documented in the last 18 months.
    </div>
  </div>`;
...
function renderConditionCards(summaries) {
  ...
  const cards = summaries
    .map((s) => {
      ...
      const valHtml =
        latest && !isNaN(Number(latest.value))
...
function buildRecalls(recalls, registers, conditionSummaries) {
  ...
  if (registers.length) {
    ...
    const regsGrid = ordered
      .map((r) => {
        const rb = reviewBadge(r);
        const reviewLine = `<span class="badge ${rb.cls}">${esc(rb.text)}</span>`;
        const dateStr = r.lastReview
          ? r.lastReview.date.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
          : '—';
  ...
  ${
    recalls.completed.length
      ? `<div class="card">
    <div class="card-title">Completed recalls (${recalls.completed.length})</div>
    <p style="font-size:13px;color:#768692">${recalls.completed
      .slice(0, 5)
      .map((r) => esc(r.code))
      .join(' · ')}${recalls.completed.length > 5 ? ` · +${recalls.completed.length - 5} more` : ''}</p>
  </div>`
      : ''
  }`;
