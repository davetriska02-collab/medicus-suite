...
// ---- 2026-06-11 red-team additions: positive assertions ----

// stroke-tia: new lay phrasings
expectMatch("I can't get my words out at all", ['stroke-tia']);
expectMatch("I can't speak properly since this morning", ['stroke-tia']);
expectMatch('she has lost the ability to speak', ['stroke-tia']);
expectMatch('both my arms have suddenly gone weak', ['stroke-tia']);
expectMatch('both arms gone numb just now', ['stroke-tia']);

// sepsis: systemic illness phrasings
expectMatch('temperature and feeling absolutely terrible', ['sepsis']);
expectMatch('hot and cold sweats all night', ['sepsis']);
expectMatch('shaking uncontrollably and have a high fever', ['sepsis']);

// meningitis: purple/blotchy rash and glass-test phrasings
expectMatch('temperature and a blotchy rash on her legs', ['meningitis']);
expectMatch("the spots won't fade under a glass", ['meningitis']);
expectMatch('a purple rash with fever', ['meningitis']);

// chest-pain: jaw/neck referred pain and indigestion mimics
expectMatch('jaw pain and arm pain', ['chest-pain']);
expectMatch('pain in my left arm and sweating', ['chest-pain']);
expectMatch('indigestion and jaw pain', ['chest-pain']);
expectMatch('heartburn with sweating', ['chest-pain']);
expectMatch('neck pain with shoulder pain', ['chest-pain']);

// thunderclap: neck stiffness with headache
expectMatch('sudden headache and stiff neck this morning', ['thunderclap']);
expectMatch('worst headache with neck pain', ['thunderclap']);

// cauda-equina: lay retention/sensation phrasings
expectMatch("I've lost all feeling down below", ['cauda-equina']);
expectMatch("can't feel when I go to the toilet", ['cauda-equina']);

// uti amber: acute retention without back-pain context
expectMatch("I can't pee at all and nothing comes out", ['uti']);
expectMatch('struggling to pass water since this morning', ['uti']);
...
expectMatch('bitten by a tick on a walk', ['insect-bite']);
expectMatch("a bull's-eye rash around a bite", ['insect-bite']);
expectMatch('I think I might have Lyme disease', ['insect-bite']);

// shingles: burning pain on one side
expectMatch('burning pain down the left side of my chest', ['shingles']);
expectMatch('burning pain on one side of my back', ['shingles']);

// ---- 2026-06-11 red-team additions: negative controls for 3 riskiest patterns ----

// 'both arms ... weak/numb/heavy' should not fire on bilateral gradual arthritis complaints
expectMatch('both arms a bit achy from gardening today', [], ['stroke-tia']);
// 'shaking uncontrollably' must not fire on a Parkinson tremor admin query (no fever context needed
// in pattern, but check it doesn't fire stroke/other reds unexpectedly)
expectMatch(
  "shaking uncontrollably from my Parkinson's — need a medication review",
  ['sepsis'],
  ['stroke-tia', 'cauda-equina']
);
// '(indigestion|heartburn) ... sweating' must not fire cardiac alert on pure GORD description
// without arm/jaw/sweat (existing patterns fine; new literal 'heartburn and sweating' is intentional)
// Check that 'heartburn alone' does NOT fire chest-pain
expectMatch('just got heartburn after spicy food, nothing else', [], ['chest-pain']);
