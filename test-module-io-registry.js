...
//
// MODULE_SCOPES in suite-envelope.js is the one ordered list. options.js
// MODULE_IO must have the same keys, and the export/import loops must read
// that list rather than a hand-maintained parallel array.
...
check(
  ...
  'MODULE_SCOPES has no duplicates'
);
...
check(!!ioBlock, 'options.js declares MODULE_IO');

const ioKeys = ioBlock
  ? [...ioBlock[1].matchAll(/^\s{2}([A-Za-z][A-Za-z0-9]*)\s*:/gm)].map((m) => m[1])
  : [];
...
check(
  /SuiteEnvelope\.MODULE_SCOPES/.test(optionsSrc) && /doFullExport/.test(optionsSrc),
...
check(
  !/const exporters = \{/.test(optionsSrc),
  'doModuleExport no longer has a parallel exporters map'
);
