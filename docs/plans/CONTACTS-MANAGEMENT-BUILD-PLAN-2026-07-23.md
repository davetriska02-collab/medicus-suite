...
> - Two further ideas from 2026-07-26 aren't in this original plan at all: flagging when a patient shares a non-Home phone/email with one of their own linked contacts (a natural extension of the matching section's own stated caution below about shared family contact details) — **not yet built, next up as of 2026-07-29** — and flagging a number stored under the wrong type (e.g. a mobile-format number filed as "Home") — **built 2026-07-28, see CHANGELOG v3.189.0** ("Fix type" in the merge-compare panel's Phone rows).
