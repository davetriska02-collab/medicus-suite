...
This hazard log applies to all functional modules of Medicus Suite v3.60.9, namely:
...
- **Today** — morning command-centre tab (v3.60.0) aggregating existing administrative feeds (waiting room, triage load, demand counts, slot availability, last Sweep summary, recent alert log) into one view; administrative glance only — see CSN limitation 33. From v3.60.7–v3.60.9 the tab additionally shows the age of the oldest unanswered new-request (when a parseable creation timestamp is available) and a visual demand headroom meter for medical and admin demand against the configured amber/red thresholds; both are display-only arithmetic aggregations of data already surfaced elsewhere in the suite.
- **Suite chrome and onboarding (v3.57.0–v3.60.9)** — suite-wide guided tour, first-run setup checklist, command palette (Ctrl+K), view-state continuity, consolidated Notifications settings with **clinic mode** (temporary mute of the extension's own desktop pop-ups and sounds only — never strips, badges, chips or in-page surfaces; see H-028), **reception capture drafts** (≤4h local working copy — see H-029) and **resumable Sweep** (≤2h local working copy — see H-027). From v3.60.3 a **tab chooser** ("Choose tabs…" via Ctrl+K or the first-run checklist) allows per-user hiding of any suite tab — see H-012 control (h). From v3.60.2 Slot type pills gain per-type colour customisation; alert-level colours (amber/red threshold) always override user-configured colours so safety salience is not configurable away. No change to the rules engine, extraction logic, or any clinical threshold in these releases.
...
| **Initial risk** | 6 |
| **Controls / mitigations** | (a) The full automated test suite (440+ automated checks at v3.26.4, 53 test files at v3.60.9) must pass before a release tag is pushed; the CI release workflow fails closed on any failure. (b) Test files cover the rule engine, QOF year logic, QOF indicator filters, custom indicators, IO, update checker, request monitor, applicability filters, STOPP/START prescribing flags, extraction health, sentinel panel state, snapshot bridge, suite IO, custom-rule-builder round-trip, tab catalog parity, and many more. (c) **A dedicated CI test workflow (`test.yml`, v3.17.0)** runs the full suite plus defaults-consistency and syntax checks on every push and pull request — making regression detection continuous, not just at release; previously the release workflow cut releases without running the test suite. (d) `CHANGELOG.md` documents every change. (e) Version number is surfaced in the Options page and popup. (f) The auto-update mechanism alerts users to new versions but does not auto-install. (g) A CSO-approved hot-fix release can be cut within hours. |
| **Residual severity** | 3 |
| **Residual likelihood** | 2 |
| **Residual risk** | 6 — Acceptable (ALARP) |
...
No hazard has a residual risk score exceeding 9. No hazard at residual score 10 or above is open. The release of v3.60.9 is approved by the Clinical Safety Officer on the basis of this hazard log.
...
| 2026-06-13 | 3.7 | DT | Weekly safety-case sync — v3.60.2 through v3.60.9 (design-crit UI passes, Slots redesign, tab chooser, Today tab altitude features; no change to rules engine, extraction logic, or clinical thresholds). **Scope (§2) updated:** Today tab entry extended with oldest-unanswered-request age and demand headroom meter (v3.60.7–v3.60.9); suite chrome entry extended with tab chooser (v3.60.3) and Slots pill colour customisation (v3.60.2). **Updated hazards:** H-005 control (m) — RM-strip infinite poll-loop bug (commit a0d440c): `panel.js` was triggering a new poll on every `suite.requestMonitor.state` write from `pollAll()`, creating a self-sustaining API-hammering loop; fixed to react to five CONFIG keys only, matching the service worker's existing guard. H-006 test count updated from 52 to 53 test files. H-012 control (h) — tab chooser recorded as a new interruption-management surface with caution that hidden tabs do not disable monitoring rules, and mitigations (command palette access, Triage HUD, strips all unaffected). **No new hazard IDs added** (all new features fit within existing hazard scope with adequate mitigations). **FLAG for separate review:** `INTENDED-PURPOSE.md` module table has not been updated since v3.16.0 and omits Trends, Condor, Sweep, Reception, Knowledge, and Today — a dedicated update pass is required. |

## 9. Clinical Safety Officer sign-off

I confirm that I have reviewed each hazard recorded in this log, that the controls described are in place at v3.60.9, and that the residual risks are acceptable for limited distribution to named GP users who have read and accepted the Clinical Safety Notice and the full disclaimer.

**Dr Dave Triska, GMC 7534932**  
**Clinical Safety Officer, Medicus Suite**  
**Graysbrook Ltd**  
**Date:** 2026-06-13 *(weekly sync — v3.60.9; no new hazard IDs; H-005 and H-012 controls updated)*
