...
- **Renal Monitoring** (formerly ACR Trend, renamed v3.29.2) — ACR trend chart with KDIGO A1/A2/A3 band shading, co-displayed eGFR with G-stage bands, a KDIGO G×A monitoring-frequency cell, and action banners for ACR ≥70 referral trigger, ACR doubling since prior reading, and KDIGO category crossing — all based on NICE NG203 (v3.25.0)
...
| **Initial risk** | 6 |
| **Controls / mitigations** | (a) The full automated test suite (570+ automated checks at v3.29.3, across 20 test files) must pass before a release tag is pushed; the CI release workflow fails closed on any failure. (b) Test files cover the rule engine, QOF year logic, QOF indicator filters, custom indicators, IO, update checker, request monitor, applicability filters, STOPP/START prescribing flags, extraction health, sentinel panel state, snapshot bridge, suite IO, custom-rule-builder round-trip, drug brand coverage (264 assertions; cross-rule collision check), prescribing flags, and import hardening (439 assertions covering malformed thresholds, proto-pollution, oversized files, and monitoring-disable warnings). (c) **A dedicated CI test workflow (`test.yml`, v3.17.0)** runs the full suite plus defaults-consistency and syntax checks on every push and pull request — making regression detection continuous, not just at release; previously the release workflow cut releases without running the test suite. (d) `CHANGELOG.md` documents every change. (e) Version number is surfaced in the Options page and popup. (f) The auto-update mechanism alerts users to new versions but does not auto-install. (g) A CSO-approved hot-fix release can be cut within hours. |
| **Residual severity** | 3 |
| **Residual likelihood** | 2 |
| **Residual risk** | 6 — Acceptable (ALARP) |
...
| **Initial likelihood** | 2 (Unlikely) |
| **Initial risk** | 10 |
| **Controls / mitigations** | (a) The manifest's `host_permissions` are restricted to `*.medicus.health/*`, `*.api.england.medicus.health/*`, and the single GitHub releases path for the update checker — outbound calls to any other host require a manifest change and a new release. From v3.28.x the GitHub `host_permissions` entry was narrowed from `api.github.com/*` to the single repository path actually used, further limiting the outbound surface. (b) The update checker transmits only the version string to the GitHub releases API — no patient or practice identifiers. (c) Update checks can be disabled entirely via the Options page. (d) All Medicus API calls reuse the user's existing session cookies — no separate credentials are stored. (e) Code review and CI inspect `manifest.json` and outbound fetch calls at every release. (f) The repository is private; releases are built via the GitHub Actions release workflow. (g) Users are instructed never to install from an unofficial source. (h) The Visualiser processes PDFs entirely in-memory; no PDF content is transmitted externally. (i) **Data minimisation (v3.28.x):** full patient names are no longer persisted to `chrome.storage.local`; the waiting-room request monitor and panel notifications use initials and counts only, reducing the identifiability of any data an attacker could read from local storage. (j) **`web_accessible_resources` narrowed (v3.28.x):** the manifest `web_accessible_resources` declaration is restricted to the five files actually loaded by content scripts, reducing the extension attack surface available to hostile web pages. (k) **Message integrity (v3.28.x):** `runtime.onMessage` handlers now validate `sender.id` against the extension's own ID before processing; the MAIN-world task-list bridge validates message shape, enforces a 10 MB import cap, and is rate-limited to prevent denial-of-service via message flooding. (l) **Supply-chain integrity (v3.28.x):** `vendor-versions.json` records SHA-256 hashes for all vendored files; practice-code is validated against an allowlist before being used in an API fetch URL. `SECURITY-AUDIT.md` (red-team pass, v3.28.x) documents the full finding list and mitigations. |
| **Residual severity** | 5 |
...
| **Initial risk** | 9 |
| **Controls / mitigations** | (a) The prompts are explicitly supplementary to Medicus's own prescribing-safety systems, which remain the primary control. (b) STOPP/START detection is deterministic, name-based, and worded as a *review* prompt ("review need / gastroprotection", "consider deprescribing") — never a recommendation of a specific drug, dose, or change. (c) Topical NSAIDs are excluded from the combination logic; the age-gated benzodiazepine check fires only when age is *known* ≥80. (d) The risk-tool chip is signpost-only — it links to the official calculators and lists the inputs they need; the suite computes no score, deliberately avoiding the medical-device exposure of an unvalidated reimplementation (the extractors cannot supply cholesterol ratio / smoking / ethnicity). (e) Pharmacy First snippets state each pathway's age/sex gateway, are worded "consider … if eligible", carry red-flag safety-netting, and instruct the clinician to confirm eligibility — the signposting decision remains the clinician's. (f) Verification against the source record is required by the disclaimer before any action. (g) The deterministic logic is unit-tested (`test-prescribing-flags.js`) and the Pharmacy First detection rules were checked against firing / non-firing examples. (h) **NSAID prescribing-flag coverage (v3.29.1):** the oral NSAID detection set in `evaluatePrescribingFlags` (content.js) has been extended to the complete current UK list (adding tenoxicam, sulindac, dexketoprofen, tiaprofenic acid, tolfenamic acid, fenoprofen), reducing the false-negative rate for NSAID-combination prescribing-flag chips in the live Triage Lens panel. Seven new assertions in `test-prescribing-flags.js` (22 passing total) regression-guard the extended set. |
| **Residual severity** | 3 |
| **Residual likelihood** | 2 |
| **Residual risk** | 6 — Acceptable (ALARP) |
...
### H-022 — Malicious or malformed ruleset import silently disables clinical monitoring

| Field | Value |
|-------|-------|
| **Hazard ID** | H-022 |
| **Description** | A crafted or malformed suite backup (`.json` import) could overwrite legitimate drug-monitoring thresholds with invalid values (zero, non-finite, or out-of-range numbers), set enum fields to unrecognised strings, or set `enabled: false` on monitoring rules — causing clinical safety alerts to silently fail to fire after import, with no visible indication to the user that monitoring rules have been degraded. |
| **Potential causes** | Attacker with write access to a backup file or to the import pathway; user importing an untrusted backup shared from outside the practice; backup corrupted during file transfer; user importing a backup from a version with incompatible schema. This was identified as a High finding (F1) in the red-team security audit (`SECURITY-AUDIT.md`, v3.28.x). |
| **Affected users / components** | All users on workstations where a malicious or malformed backup has been imported. Components: `engine/ruleset-io.js`, `shared/io/suite-envelope.js`, `options/options.js`. |
| **Initial severity** | 4 (Major — selective or wholesale disabling of drug-monitoring alerts without user awareness; functionally equivalent to permanently hiding monitoring rules on that workstation) |
| **Initial likelihood** | 2 (Unlikely — requires access to the import file; backups are practice-internal; import is a deliberate user action) |
| **Initial risk** | 8 |
| **Controls / mitigations** | (a) **Import validation (v3.28.x):** `engine/ruleset-io.js` now hard-validates every `check.*` threshold using `Number.isFinite`, enum fields against known-valid allowlists (`kind`, `operator`, `comparator`, `direction`), and interval values (`intervalDays`, `dueSoonDays`) against finite positive bounds before merging into storage — any non-finite, out-of-range, or unrecognised value causes the import to fail with a visible error, not a silent partial apply. (b) **Proto-key stripping:** `__proto__` and `constructor` keys in imported objects are stripped before the merge step, preventing prototype-pollution via the import path. (c) **Import preview warning:** when a backup would set `enabled: false` on monitoring rules, the import preview panel shows a conspicuous warning before the user confirms — the user cannot inadvertently accept rule suppression without seeing it. (d) **10 MB import cap:** oversized files are rejected before parsing to prevent memory-exhaustion attacks. (e) **`test-import-hardening.js` (v3.28.x):** 439 assertions cover malformed threshold values, non-finite numbers, proto-pollution attempts, oversized files, unknown fields, and the monitoring-disable warning — CI fails if any bypass is introduced. (f) The import button requires user interaction in the Options page; there is no auto-import mechanism. (g) Users are instructed not to import backups from untrusted sources. |
| **Residual severity** | 4 |
| **Residual likelihood** | 1 |
| **Residual risk** | 4 — Broadly acceptable |
| **Acceptability** | Accepted. Hard validation on import converts silent rule suppression into an explicit import failure. Any report of monitoring alerts failing to fire after an import is treated as a significant safety event and triggers immediate CSO review. |
...
| H-022 | Malicious/malformed import silently disables monitoring | 4×2 | 8 | 4×1 | 4 | Broadly acceptable |

No hazard has a residual risk score exceeding 9. No hazard at residual score 10 or above is open. The release of v3.29.3 is approved by the Clinical Safety Officer on the basis of this hazard log.
...
| 2026-06-06 | 3.4 | DT | Synchronised to v3.29.3. **New hazard:** H-022 (malicious or malformed ruleset import silently disables clinical monitoring — identified as High finding F1 in the red-team security audit, v3.28.x; mitigated by hard import validation, proto-key stripping, monitoring-disable preview warning, 10 MB cap, and 439-assertion `test-import-hardening.js`; residual risk 4×1 = 4, broadly acceptable). **Scope updated:** ACR Trend renamed Renal Monitoring (v3.29.2, content unchanged); comprehensive UK brand-name coverage added to all 24 drug-monitoring rules (v3.26.7–v3.28.1); QOF Obesity 2026/27 area added (v3.29.0, OB004/OB005 DISABLED pending CSO review); NSAID set completed in live Triage Lens and `alert-library.json` (v3.29.1). **Updated hazards:** H-002 (added (n) drug brand-name coverage v3.26.7–v3.28.1 with 264-assertion `test-drug-brand-coverage.js` CI guard, injectable-MTX exclusion removal; (o) QOF Obesity 2026/27 OB register with draft indicators); H-006 (test count updated to 570+, 20 test files); H-009 (added (i)–(l): data minimisation, `web_accessible_resources` narrowed, message integrity, supply-chain integrity — all from v3.28.x red-team hardening pass); H-016 (added (g) NSAID set completion in `alert-library.json` v3.29.1); H-019 (added (h) NSAID prescribing-flag coverage completion in content.js v3.29.1). No residual risk scores have changed. CSO sign-off updated. |

## 9. Clinical Safety Officer sign-off

I confirm that I have reviewed each hazard recorded in this log, that the controls described are in place at v3.29.3, and that the residual risks are acceptable for limited distribution to named GP users who have read and accepted the Clinical Safety Notice and the full disclaimer.
