...
From v3.60.2–v3.60.9 further user-experience improvements were made with **no change to the rules engine, extraction logic, or any clinical threshold**: a **Slots** module glanceable redesign (hero card, threshold-aware type pills with per-type colour customisation — alert-level colours always override custom colours so safety salience is not configurable away; share micro-bars); a **tab chooser** ("Choose tabs…" accessible via Ctrl+K or the first-run setup checklist, with role presets — GP, Reception, Practice manager, Everything) allowing per-user hiding of any suite tab — see limitation 35; **Today** tab altitude additions showing the age of the oldest unanswered new-request across all request buckets and a visual demand headroom meter for each demand stream against the configured amber/red thresholds (v3.60.7–v3.60.9); and a fix for an infinite RM-strip poll loop in `panel.js` that was hammering the task-list API continuously rather than on the intended 60-second interval — see H-005 control (m).
...
35. **Hiding the Monitoring tab via the tab chooser removes proactive chip visibility on that tab — it does not disable monitoring rules.** From v3.60.3 users can hide any suite tab via the "Choose tabs…" command palette action, the role presets (GP, Reception, Practice manager, Everything) in the first-run setup checklist, or the per-tab toggle cards accessible from Ctrl+K. A user who hides the Monitoring (Sentinel) tab will not see Sentinel chips in that tab's view without navigating back to it via the command palette. The Triage Lens HUD on the patient record, the waiting-room/triage/demand strips in the panel header, and all other clinical surfaces remain fully visible regardless of tab visibility. The Monitoring tab can be restored at any time via the command palette; at least one tab is always visible. The `suite.hiddenTabs` preference is per-user and cannot be set by a practice-profile push — only the user themselves can hide or unhide their tabs. The deploying practice must brief users that hiding the Monitoring tab does not disable monitoring rules, that chips are not displayed while the tab is hidden, and that the "no chip ≠ all clear" principle from limitation 22 applies equally to chips that are not visible because the tab is hidden.
...
3. **Brief users.** Every user must be briefed on:
   ...
   - the tab chooser — hiding the Monitoring tab removes proactive chip visibility but does not disable monitoring rules; the tab is always reachable via the command palette; "no chip ≠ all clear" applies when the tab is hidden (section 7, limitation 35)
...
| **Release gating** | GitHub Actions release workflow runs the full automated test suite (53 test suites at v3.60.9, covering applicability-filter, STOPP/START prescribing-flag, QOF indicator filters, extraction-health, sentinel panel-state, custom-rule-builder round-trip, drug-brand coverage, service-worker behaviour, API-client contracts, PINCER parity, reception pathways, ACB scores, STOPP/START v3 flags, triage-rule patterns, action packs, patient passport, pre-consultation brief, hidden-rule resurfacing, tab catalog parity, and many more) and fails closed on any test failure. A dedicated `test.yml` CI workflow also runs the full suite on every push and pull request (v3.17.0). A release is cut only by pushing a signed tag. |
...
I confirm that this notice fairly represents the clinical safety position of Medicus Suite v3.60.9; that the residual risks recorded in `docs/HAZARD-LOG.md` are acceptable for limited distribution to named GP users under the conditions set out in section 9; and that the controls described are in place at this release.

**Dr Dave Triska, GMC 7534932**  
**Clinical Safety Officer, Medicus Suite**  
**Graysbrook Ltd**  
**Date:** 2026-06-13 *(weekly sync — v3.60.9; no new hazard IDs; H-005 and H-012 controls updated; limitation 35 added)*
...
| 2026-06-13 | 3.6 | DT | Weekly safety-case sync — v3.60.2 through v3.60.9 (design-crit UI passes, Slots redesign, tab chooser, Today altitude features; no change to rules engine, extraction, or clinical thresholds). **Section 2 (Intended purpose):** added paragraph covering v3.60.2–v3.60.9 changes — Slots glanceable redesign, tab chooser (v3.60.3), Today tab altitude additions (oldest unanswered request, demand headroom meter, v3.60.7–v3.60.9), and RM-strip infinite poll-loop fix. **Section 7:** added limitation 35 (tab chooser — hiding Monitoring tab removes proactive chip visibility but does not disable rules; command palette always accessible; "no chip ≠ all clear" applies). **Section 9.1:** briefing list extended with limitation 35 (tab chooser). **Section 11:** test count updated to 53 test suites. Updated sign-off. Aligned with `HAZARD-LOG.md` v3.7 (updated H-005 control m, H-006 test count, H-012 control h; no new hazard IDs). |
