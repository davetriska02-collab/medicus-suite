...
- The vaccination eligibility chips (v3.26.0, extended with PPV23/shingles/RSV at v3.46.0) infer eligibility and status from coded data as a prompt to check — they do not interact with the National Immunisation Management Service (NIMS) or any external vaccination record.
- The **Action Packs** feature (v3.48.0) generates copy-ready communication templates (blood forms, recall SMS/letters, pharmacist tasks) from the current Sentinel chip set. All template dispatch is manual and user-initiated; the extension transmits nothing automatically. The feature is a structured drafting aid, not a communication or prescribing system.
- The **Pre-consultation Brief** (v3.49.0) is a display-layer summary of the top-ranked Sentinel chips; it applies no clinical reasoning beyond what the Sentinel module already applies.
- The **Patient Passport** (v3.50.0) is a plain-English reformatting of the current Sentinel chip set for patient use; it performs no clinical reasoning and does not transmit data externally.
- The **SMR Workstation** (v3.51.0) applies arithmetic ACB burden scoring and a deterministic STOPP/START v3 criterion check against PDF text — these are threshold comparisons and pattern-matched drug lists, not clinical recommendations. The software does not recommend any medication change. STOPP criteria worded as "consider deprescribing" are prompts to review, not prescribing instructions.
- The **Triage Lens expanded red-flag rules** (v3.44.x–v3.52.0) are text-pattern detections on incoming request text, surfaced as prompts to escalate. They do not constitute a triage decision made by the software; the clinical triage decision remains with the clinician or trained senior staff.
...
23. **Vaccination eligibility and status are inferred from coded data.** Flu and COVID-19 eligibility chips are derived from QOF register membership, problem-list entries, medications, BMI, and age using JCVI/UKHSA 2025/26 criteria. DUE / GIVEN / DECLINED status is inferred from coded problems, observations, and journal entries within the current season window. From v3.46.0 three one-off (lifetime) vaccination rules were added: pneumococcal (PPV23, age ≥65, Green Book ch 25), shingles (Shingrix, phased cohort 65-79, Green Book ch 28a), and RSV (age 75-79, Green Book ch 27a); a bug in which declined-vaccination coded records were misclassified as vaccination-given was corrected at the same time. Vaccinations administered outside the practice (pharmacy, community hub, hospital, other practice, or years before the current GP registration) may not be recorded locally. For one-off lifetime vaccines (PPV23, shingles, RSV) this is especially important: the patient may have been vaccinated years or decades earlier without a local record. Every vaccine chip carries a "DOUBLE-CHECK ELIGIBILITY" note and matched evidence; vaccination status must be verified against the Medicus record and, where relevant, national vaccination records (NIMS/PCSE/SOARIS) before any clinical action.
...
### Pre-consultation Brief (v3.49.0)

28. **The pre-consultation brief compresses the chip list to the top 4 ranked signals.** The collapsible brief card at the top of the Sentinel panel surfaces up to 4 risk-ranked action signals and an overflow count (+N more below) when the full list is longer. Clinicians must not rely on the brief card alone — it is a 30-second orientation aid, not a complete safety summary. The "no chip ≠ all clear" principle applies equally to chips not shown in the brief card. The full chip list remains visible by scrolling or expanding the brief.

### Action Packs (v3.48.0)

29. **Action Pack templates contain patient-identifiable data and must be treated as patient data.** Blood-form requests, recall SMS/letters, and pharmacist-task templates generated by the Action Packs feature include the patient's name, date of birth, NHS number, and clinical detail. Template text is placed on the system clipboard by the user's deliberate action; the extension does not transmit any template content automatically. The user must: verify the patient against the Medicus record before generating a pack; review the template before dispatching it; confirm the intended recipient; and use only information-governance-approved channels. Clipboard content management is the responsibility of the practice's IT security and information-governance policy.

### Patient Passport (v3.50.0)

30. **The Patient Passport is a memory aid for patients, not a clinical assessment, and must be accompanied by an oral explanation.** The printable plain-English health summary reflects what the Sentinel rules engine found due at the time of printing, subject to all limitations at items 1–9 above. It does not claim to be a complete health assessment, and "nothing currently due" does not mean "all is well". The GP must review the passport before handing it to the patient and should accompany it with an appropriate oral explanation. The GP retains full clinical responsibility for how the summary is communicated and for any decisions the patient makes based on it. Patient-identifiable data stored transiently to generate the passport is deleted from device storage immediately after the print page renders (consume-on-read).

### SMR Workstation — Visualiser (v3.51.0)

31. **The SMR Workstation ACB scores and STOPP/START criteria are a starter set pending CSO verification.** The Structured Medication Review tab in the Patient Record Visualiser displays ACB burden scores (Boustani scale) and STOPP/START v3 (2023) criteria flags derived from the Visualiser PDF export. These outputs inherit all PDF staleness (limitation 10) and parsing-completeness (limitation 11) limitations. ACB scoring is drug-name based and will miss brands or abbreviated entries not in the reference set. Age-gated and eGFR-gated STOPP/START criteria fail-closed when age or eGFR cannot be parsed from the PDF. The implemented STOPP/START set (10 STOPP + 3 START criteria) is an intentionally limited implementable subset; absence of a flag does not mean the criterion is not applicable. **The ACB reference set and STOPP/START criteria have not yet been formally verified by the CSO against current Boustani guidance and STOPP/START v3 (2023); until that verification is recorded, the SMR Workstation tab should be treated as a reference aid only, not as a clinical decision support tool.** Verify every ACB score and STOPP/START flag against the live Medicus prescribing record before any medication is changed.

### Triage Lens expanded red-flag rules (v3.44.x–v3.52.0)

32. **Triage Lens red-flag rules are text-pattern matching and do not cover all clinical phrasings of emergency presentations.** Since v3.44.x the request-queue rule set includes 17 explicit red emergency-presentation rules (stroke/TIA, sepsis, anaphylaxis, meningitis, DKA/HHS, and others). These rules fire when the request text matches the expected patterns; they will not fire when a patient or caller uses colloquial, indirect, or atypical phrasing. Absence of a red chip does not mean the presentation is safe to manage routinely. The practice's own clinical triage protocol remains the primary safety gate; the Triage Lens supplements but does not replace that protocol. Practice staff must not rely solely on the Triage Lens to identify emergency presentations — they must apply their own professional training to every incoming request.
...
| **Release gating** | GitHub Actions release workflow runs the full automated test suite (50 test files at v3.54.0, covering rule-engine evaluation, QOF indicator filters, STOPP/START prescribing flags, ACB scores, STOPP/START v3 criteria, action packs, passport core, pre-consultation brief, vaccine rules, PINCER parity, extraction baseline, evaluation trace, service-worker contracts, and all existing checks from earlier releases) and fails closed on any test failure. A dedicated `test.yml` CI workflow also runs the full suite on every push and pull request (v3.17.0). A new `check-doc-versions.js` step ensures safety-doc product versions track the manifest major.minor (v3.53.0). A release is cut only by pushing a signed tag. |
...
I confirm that this notice fairly represents the clinical safety position of Medicus Suite v3.54.0; that the residual risks recorded in `docs/HAZARD-LOG.md` are acceptable for limited distribution to named GP users under the conditions set out in section 9; and that the controls described are in place at this release. The CSO-verification action against H-023 (SMR Workstation ACB/STOPP/START criteria) must be completed before that tab is used clinically beyond limited evaluation.
...
| 2026-06-11 | 3.4 | DT | Synchronised to v3.54.0. Updated intended purpose (section 2) to describe features added since v3.26.4: PPV23/shingles/RSV one-off vaccine rules and declined-before-given fix (v3.46.0); extraction-drift detection (v3.47.0); Action Packs (v3.48.0); Pre-consultation Brief (v3.49.0); Patient Passport (v3.50.0); SMR Workstation in Visualiser (v3.51.0); Triage Lens 77-rule expansion including emergency red-flag rules (v3.44.x) and DKA/HHS red flag with engine hardening (v3.52.0). Updated regulatory status (section 4) with new features. Updated limitation 23 (vaccination — added one-off vaccine rules and declined-before-given fix). Added known limitations 28 (pre-consultation brief — top-4 truncation), 29 (Action Packs — PII in templates), 30 (Patient Passport — patient memory aid), 31 (SMR Workstation — ACB/STOPP/START starter set, CSO verification required), 32 (Triage Lens emergency red-flag text-pattern limitations). Updated test count (50 test files). Updated sign-off. Aligned with `HAZARD-LOG.md` v3.5 (new H-022 to H-025; updated H-002, H-003, H-005, H-007, H-009, H-016, H-017, H-019, H-020). |
