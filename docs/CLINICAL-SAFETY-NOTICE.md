...
27. **The Reception module's guided capture is a structured history-taking aid for non-clinical staff — it is NOT a triage tool and makes no clinical decisions.** The capture pathways (`rules/reception-pathways.json`) present a fixed, identical-every-time question set per presenting problem, with red-flag questions asked first. The following constraints apply:
    ...
    - **Captured answers, generated text, and taker initials are in-memory only** and discarded when the panel closes. Stored configuration is limited to pathway enable flags, pathway content, and the chip filter (`reception.*` keys, included in suite backups).
...
| **Release gating** | GitHub Actions release workflow runs the full automated test suite (50 test suites at v3.55.0, covering applicability-filter, STOPP/START prescribing-flag, QOF indicator filters, extraction-health, sentinel panel-state, custom-rule-builder round-trip, drug-brand coverage, service-worker behaviour, API-client contracts, PINCER parity, reception pathways, ACB scores, STOPP/START v3 flags, triage-rule patterns, action packs, patient passport, pre-consultation brief, hidden-rule resurfacing, and many more) and fails closed on any test failure. A dedicated `test.yml` CI workflow also runs the full suite on every push and pull request (v3.17.0). A release is cut only by pushing a signed tag. |
...
I confirm that this notice fairly represents the clinical safety position of Medicus Suite v3.56.0; that the residual risks recorded in `docs/HAZARD-LOG.md` are acceptable for limited distribution to named GP users under the conditions set out in section 9; and that the controls described are in place at this release.
