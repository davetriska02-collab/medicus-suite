...
- **Signed .crx packaging for IT-managed installs** — `npm run pack:crx` builds a signed CRX3 package plus an `update.xml` policy feed, releases attach both when the signing secret is set, and docs/IT-DEPLOYMENT.md gives IT the extension ID and force-install policy strings.
