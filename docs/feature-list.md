...
**Version:** v3.153.0
**Generated:** 2026-07-04 (automated)
...
- 27 drug-monitoring rules, 52 QOF indicators, 13 QOF registers, 5 vaccine rules, 44 bundled investigation result rules, and 26 starter alerts in the prescribing-safety library
...
- **Morning sweep**: last sweep summary (time and action-needed count) or a prompt to run it, plus a plain-English "N of M booked patients have checks due — open the sweep for names" line (neutral zero state; the not-yet-run state is unchanged and never implies all-clear)
...
- **Visible from any tab**: the open patient's red/amber monitoring count shows on the panel header's "Monitoring →" button (amber, red if any chip is red), with a fallback badge on the Monitoring nav tab when the waiting-room strip is hidden — an unavailable snapshot shows no badge, never a false all-clear
...
- **QOF points-at-risk prioritiser**: ranks gaps by indicator points with a CVD-prevention subtotal, showing each indicator's name next to its code (e.g. "DM037 — …") rather than a bare code
- **Named skipped entries**: appointments the sweep couldn't check (no patient UUID) are listed in a collapsible amber list — time · clinician · raw name — with an explicit "never checked, not an all-clear" caveat
...
- **2WW / Faster-Diagnosis safety-net worklist**: suspected-cancer referrals still showing Incomplete, oldest-first with day-ages and watch/overdue severity; patient names wrap onto their own line rather than truncating, and the card carries a threshold legend ("watch ≥ 14d · overdue ≥ 21d", values read live from the API, not hardcoded)
...
- Each pathway tile has a "Leaflet →" link that jumps straight to the Leaflets tab with the matching NHS A-Z search already run
...
- Reserved "Locum brief" category pins first after "All", styled distinctly, with an empty-state hint and a suggested option in the Add form — the locum's day-one who's-who/escalation card, practice-authored
...
**Triage Lens — request queue** (all Medicus pages): overlays the patient request queue with semantic triage chips. Red = same-day or 999; amber = urgent; info = supplementary; green = confidently routine (practice-admin / pharmacy-process queries, so the safe tail can be swept in one pass — green ranks below every clinical kind, so a symptom co-occurring with an admin phrase always keeps its red/amber chip on top). 80 built-in rules across chest pain, sepsis, stroke/TIA, anaphylaxis, obstetric emergencies, mental health crisis, paediatric red flags, 2WW cancer patterns, and common acute presentations. When several rules match one request the chips rank by severity and collapse to the top chip plus a "+N" overflow; clicking a chip shows the exact sentence that triggered it (matched term highlighted) and the rule's attached actions — guidance links, copy-ready snippets, notes. Negated or historic mentions ("no chest pain", "UTI last year") are visually demoted, never suppressed. A request matching an NHS Pharmacy First pathway shows a green "Pharmacy First" chip when the patient is age-eligible, with a prepare-only redirect draft; a matched pathway also offers a missing-info "ask-back" (the red-flag questions the request hasn't answered) as a prepare-only draft, and surfaces any red flag the patient volunteered as a 999/duty escalation note — decision support only, never auto-triage or auto-send. New built-in rules reach existing users automatically; rules deliberately deleted stay deleted.

**Triage Lens — investigation results queue** (Medicus Investigation Results filing queue): decorates each pending result row with per-row severity chips — Urgent (red, lab's own flag), N abnormal (amber, lab's above/below-reference flags), Under-prioritised (red, result severity exceeds assigned priority), and Unmatched patient (amber). A live status bar summarises the queue ("3 red · 7 amber · 22 clear · checking 8…") with a jump-to-next-red button and a Focus-alerts dimming toggle; alert rows also carry a red/amber left-edge tint for at-a-glance scanning. The lens is honest about what it hasn't checked: a row whose check failed shows a grey "?" chip and is retried automatically, rows still being checked are counted in the bar, and "clear" is only ever said of a result that was actually assessed. Chips answer "how bad?" without opening the task: red/amber chips carry a trend arrow when a prior value exists, and clicking a chip opens a detail popover with the value, reference range, sample date, the rule that fired (with its threshold), the prior value, and any guidance actions attached to the rule. Opening the task itself shows a slim verdict banner echoing the same evaluation. Result rules can carry the same actions as request rules (pathway links, snippets) so a K⁺ chip can open the practice's hyperkalaemia protocol. User-authored analyte threshold rules and text-classification rules (e.g. MSU/urine culture) can escalate severity but can never lower or suppress a laboratory's own urgent or abnormal flag. Rules come in four kinds — threshold, text-classification, multi-condition combo, and **delta** (grade on change over time, e.g. a rising creatinine or falling Hb against the report's own history). A rule is **skipped and flagged (a grey "unit?" chip) rather than mis-graded** when its unit is incompatible with the unit the lab actually reported; an unmatched non-numeric positive result ("Positive"/"Detected") surfaces an amber "?" chip instead of vanishing. Rules can be scoped to a specimen (fail-open when absent) and to **patient context** (age/sex) — context gates fail closed, never suppressing a base match. Built-in result rules ship enabled and CSO-reviewed — biochemistry/haematology thresholds (eGFR, potassium incl. a 6.0–6.4 amber band, sodium high & low, absolute creatinine/AKI + creatinine-rise delta, glucose high & low, ALT/AST, CRP, WCC, calcium, magnesium, haemoglobin, platelets, INR, HbA1c, lithium, digoxin, TSH, ferritin, B12, FIB-4) and microbiology/imaging text classifiers (blood/urine/throat/wound/ear/genital culture, stool MC&S, H. pylori, STI NAAT, EBV serology, histology, ultrasound, bowel-screening); new custom rules arrive disabled and require clinician review before firing. **Keyboard triage**: j/k (or arrow keys) move a cursor through the queue, Enter opens the cursor row, n jumps to the next red/amber row — ignored while typing (pref, default on). **Seen-dimming** (opt-in, default off) dims rows whose task was opened this session so unworked rows stand out; it never dims a red/amber row and auto-undims the moment a row re-grades to an alert. An **"all-normal, fileable" ✓ marker** flags rows whose report is confidently all-normal with no filing blockers, reusing the Lab Filing gate — fail-closed, and it files nothing itself, only marks.

**Prescribing workflow button** (prescription-request task overview): a one-click "send to routine prescriptions" button that re-assigns the task to the configured prescribing team by driving Medicus's own re-assign UI. Configurable team and commit mode (confirm/manual/auto); never makes network calls or reads patient-data field values. Every outcome — committed, highlighted for manual send, or aborted — is recorded in a machine-local audit log and mirrored to the Clinical Event Ledger.
...
- **v3.153.0 (2026-07-04)** — **Practice-panel wishlist wave 2** (the 1–2-day roadmap items from the same appraisal): Sweep gains a nurse clinic-prep worklist (the day's action-needed findings bucketed into bloods to draw, non-blood checks, vaccines to fetch and reviews to book, with a printable prep list) and an optional QOF £ projection (practice enters its own £/point figure — no shipped price; total and CVD subtotal shown in £ beside points); Submissions and Activity gain month-to-date vs same-day-of-prior-month comparisons (spans always shown; per-clinician deltas in Activity) and Activity a "Per session" toggle dividing totals by each clinician's non-cancelled session count (count always shown; never a silent guess); Condor Pulse adds a calendar Month view with its usual snapshot-coverage disclosure; Referrals safety-net rows gain a prepare-only, letterheaded 2WW chase-letter draft (copy-to-clipboard, never auto-sent); the Record tab gains a printable clinician-facing SMR prep pack (problems, dosed meds, per-rule monitoring status, ACB/STOPP-START prompts, QOF gaps, latest observations — with the mandatory allergies/immunisations gap block and verify-before-prescribing framing); and the panel gains keyboard-first navigation (1–9 tab jumps, g-chords, / to search, ? shortcuts cheat-sheet)
- **v3.152.0 (2026-07-03)** — **Practice-panel wishlist wave 1** (first build wave from the whole-suite feature-wishlist appraisal): Sweep QOF points-at-risk rows now show each indicator's name next to its code, and list named skipped appointments (time · clinician · name, "never checked, not an all-clear" caveat); Today gains a "N of M booked patients have checks due" line; the open patient's monitoring count now shows on the header's "Monitoring →" button, or a nav-tab badge, from any tab; 2WW safety-net rows no longer truncate patient names and carry a watch/overdue threshold legend; Reception pathway tiles link straight to the matching Leaflets search; Knowledge gains a pinned "Locum brief" category; and setup wording plus the guided tour were refreshed to cover recently-added features (TOUR_VERSION 8→9)
- **v3.151.0 (2026-07-02)** — **Triage Lens — Workload off the GP (Phase 4)**, CSO-reviewed: keyboard triage (j/k/arrows, Enter, n-to-next-alert) on the results queue; opt-in seen-dimming that never dims a red/amber row and auto-undims on re-grade; an "all-normal, fileable" ✓ marker reusing the Lab Filing gate (fail-closed, files nothing itself); a green Pharmacy First divert chip on age-eligible matched requests with a prepare-only redirect draft; a missing-info ask-back listing the pathway's unanswered red-flag questions, plus a volunteered-red-flag escalation note; and green "confidently routine" chips for non-clinical requests, always ranked below any clinical chip
- **v3.150.0 (2026-07-02)** — **Triage Lens — Smarter grading (Phase 3)**, every new/changed threshold independently sourced and CSO-signed-off: a unit-mismatch guard that skips and flags rather than misgrades; text rules can now reach red for designated positive findings; an amber safety-net chip for unmatched non-numeric positive results; negation/historic-mention demotion on request chips; patient-context gates (age/sex) on result and request rules, fail-closed; a new delta/trend rule kind. New shipped result-rule coverage: hypernatraemia, absolute creatinine/AKI plus a creatinine-rise delta, glucose high & low, ALT/AST, CRP, WCC high, and a potassium 6.0–6.4 amber band; HbA1c ≥48 demoted red→amber (diagnostic, not acute-danger, threshold)
- **v3.149.0 (2026-07-02)** — **Triage Lens — Evidence at the chip (Phase 2)**: request-queue chips show the exact matched sentence plus the rule's attached guidance actions on click, ranked and collapsed to a "+N" overflow; result chips gain a trend arrow and a detail popover (value, range, sample date, the rule and threshold that fired, prior value); the task-overview verdict banner now echoes the queue's evaluation, keyed to the task so a slow fetch can never paint the wrong patient's verdict; result rules can carry the same guidance actions (links/snippets/notes) as request rules
- **v3.148.0 (2026-07-02)** — **Triage Lens — Trust: the screen never lies (Phase 1)**: the HUD and results queue now distinguish "not assessed" from "clear" everywhere — a grey "Not fully assessed" state instead of implying "No flags", a retried grey "?" chip on a failed per-row check, and a monitoring-due chip that preserves its last known state on a transient failure rather than clearing it; a live status bar shows red/amber/clear/checking counts with a jump-to-next-red button and a Focus-alerts dim toggle; alert rows carry a red/amber left-edge tint; machine-initiated writes (OIR auto-ticks, the routine-Rx re-assign button) are now fully audited with a Review action, mirrored to the Clinical Event Ledger
