...
- **Lab allocation canvas** — on the investigation-results queue, a reports pile grouped by who requested them, with clinician chips on the right. Staging is local; confirming writes Medicus's own bulk-reassign (who the task sits with — it does not file the result).
