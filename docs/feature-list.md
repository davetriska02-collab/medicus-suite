...
- **Allergy cleanup suite** — a "Clean up allergies?" trigger on the Clinical Summary (wherever the summary panel renders) that opens the "Organise allergies" full-screen canvas directly, with no intermediate accordion (2026-08-20). Every active allergy is a tile in one of four computed lanes — **Active**, **Junk / low-rel**, **Convert**, **Dual-coded** — plus an **End** bin. Drag junk or a "not an allergy" row onto End to stage removal; drop a dual-coded tile onto Dual-coded to stage clearing the stale legacy code; drop one duplicate onto its pair (dashed connector lines mark detected groups) to open the existing per-entry merge review; click a Convert tile to review turning an Allergy-to-X code into a dm+d substance **plus** a reaction code (the tile names the target; the seed is search text only, never a pre-selected conceptId). Arrange many, then **Finalise** writes the staged ends and tidies — merge and convert stay one-at-a-time modal reviews. Genuine allergies cannot be ended from the bin; the last "No known allergies" copy cannot be staged. The canvas owns no API — writes go through `window.AllergyCleanup`. Every action still requires explicit confirmation; nothing is auto-applied
...
- **v3.236.4 (2026-08-20)** — Organise-allergies canvas: Convert tiles for “Allergy to X” / “X allergy” name the substance + reaction target on the tile; the review seeds the reaction search with “allergic reaction”. Search seed only — nothing is pre-selected; a reaction stays optional. Adverse-reaction wording is unchanged.
