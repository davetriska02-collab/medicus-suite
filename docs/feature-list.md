...
- **Investigations (Lab Result Catalogue)** — a single, coded, per-practice catalogue of investigations, results and labs with its own settings page (practice context, browse/search, per-test edit and gated per-test review; entries arriving by backup, shared profile or import stay inert until approved on that machine). Outstanding Requests can opt into it (matching engine setting, default off). Lab Filing can now also opt into it (filing engine setting, default off, union-only with the legacy gate — can only add a blocker, never remove one) and works even with zero legacy Lab Filing profiles configured, driven entirely by the catalogue's own practice ranges, safety guards and lab-comment whitelists; needs its own hazard-log entry (H-080/H-081) and CSO review before wider rollout
...
- **v3.270.0 (25 Sep)** — Lab Filing can now opt into filing on the Lab Result Catalogue (setting, default off, union-only with the legacy gate, practice-published): works even with zero legacy profiles, driven by the catalogue's own practice ranges/guards/comment whitelists; a pending (edited-but-unapproved) range or guard now correctly blocks rather than silently falling back to the lab's own range; the lab-flag override moved from a per-result guard to one switch per test at a lab; catalogue-only filing now completes in one click instead of needing a second manual press of Medicus's own File button
