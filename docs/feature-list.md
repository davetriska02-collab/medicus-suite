...
- **Add coded entries as problems** — a floating panel on document-filing **and investigation-result filing** task pages offering a checkbox per coded journal-note entry; ticking entries and confirming creates each as a new Problem via Medicus's own create-problem endpoint, carrying the code, note text, and an onset date derived from the document date or the investigation's specimen/issued date — something Medicus's own "Codes & actions" / "Code as" flow has no action for at all. Docks top-right so it does not cover File document or File results; header is draggable.
...
- **Bulk remove/merge** — *retired as a page trigger (v3.236.14).* Ending and merging live on **Organise problems** (End bin + drop-to-merge). The write helpers remain in `problem-bulk-end.js` for tests.
...
- **v3.236.24 (2026-08-22)** — Monitoring: "Task created for X" (and the follow-up reminder confirmation) clears when the next patient loads.
