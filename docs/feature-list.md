...
- **v3.160.0 (2026-07-08)** — Practice-panel wishlist wave 2: Sweep gains a nurse clinic-prep worklist (bloods / checks / vaccines / reviews, printable) and an optional QOF £ projection from a practice-entered £/point figure; Submissions and Activity gain month-on-month comparisons (month-to-date vs the same span last month) and Activity a per-session view; Referrals safety-net rows offer a prepare-only 2WW chase-letter draft; the Record tab prints a clinician-facing SMR prep pack with its gaps stated; and the panel gains keyboard navigation (1–9, g-chords, /, ?).
