...
### Note
Companion tab for the full-tab **Note** display board (waiting-room TV or staff-room monitor). Pick a profile, edit the flap message, toggle widgets, set when the room looks busy, and open the kiosk.
- **Waiting room** (public TV) — flap message, people-waiting count, wait-time band (not a named wait in minutes), how-busy from occupancy only. The practice sets the wait-band minutes and the Busy / Very busy counts. Does not show medical/admin request counts. A dead feed fails loud rather than painting a quiet empty room.
