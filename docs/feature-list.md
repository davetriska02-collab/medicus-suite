...
- **v3.234.0 (2026-08-15)** — Weekly safety-review fix pack, part 3 (at CSO direction, hazard H-064): the Rota passcode gate gains a non-destructive recovery route — a one-time 10-character recovery code issued when a passcode is set (shown once, stored only as a hash, "write this down where a second manager can reach it"), redeemable on the unlock screen to remove a forgotten passcode without wiping the rota; legacy installs without a code are unchanged and acquire one on the next passcode change or via "Issue a new recovery code".
