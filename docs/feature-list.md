...
- **Lab allocation canvas** — on the investigation-results queue, a large unallocated-reports box grouped by who requested them, with clinician fields on the right that hold what already sits with each person (click to expand). Staging is local; confirming writes Medicus's own bulk-reassign (who the task sits with — it does not file the result).
