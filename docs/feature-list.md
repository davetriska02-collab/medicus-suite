...
**Triage Lens — request queue** (all Medicus pages): overlays the patient request queue with semantic triage chips. Red = same-day or 999; amber = urgent; info = supplementary. 77+ built-in rules across chest pain, sepsis, stroke/TIA, anaphylaxis, obstetric emergencies, mental health crisis, paediatric red flags, 2WW cancer patterns, and common acute presentations. When several rules match one request the chips rank by severity and collapse to the top chip plus a "+N" overflow; clicking a chip shows the exact sentence that triggered it (matched term highlighted) and the rule's attached actions — guidance links, copy-ready snippets, notes. New built-in rules reach existing users automatically; rules deliberately deleted stay deleted.
