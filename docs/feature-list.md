...
- **Triage queue overlay** — age/status decoration chips, drug-monitoring and result-triage chips, and keyword-based red-flag detection with linked actions, on both the main triage queue and the investigation-results queue; compresses into a compact "pulse" display on busy queues (rail + named headline + why/act trays). Duty-doctor cockpit extras: Created-column SLA / contract-clock chips, a dashed “record not checked” rail, a hollow diamond when the headline came from the record (monitoring due / pending abnormal lab), a status-bar huddle of pulsed-red / unchecked / SLA-today counts, and an Act-tray (Book creates a reception task for the next green day — does not hold a slot; Pharmacy First / Ask-back drafts; local Park).
